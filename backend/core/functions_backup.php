            ...
            foreach ($subsites_rows as $sub) {
                ...
                foreach ($agents_rows as $agent) {
                    ...
                    foreach ($dates as $date) {
                        ...
                        if ($sJ === 'A' || in_array($sJ, ['ABANDON', 'DEMISSION'])) {
                            $absences++;
                            $reason = $sJ;
                            ...
                            $entrant_sortant_count++; // SORTANT génère une retenue (inchangé)
                        }
                        ...
                        if ($sN === 'A' || in_array($sN, ['ABANDON', 'DEMISSION'])) {
                            $absences++;
                            $reason = $sN;
                    ...
                    if ($is_special) {
                        foreach (['J', 'N'] as $main_key) {
                            foreach ($dates as $date) {
                                ...
                                if ($is_special_rest && ($main_status === '1' || strpos($main_status, 'F_') === 0 || $main_status === 'COST')) {
                                    $tp_extra_days++;
                                }
                            }
                        }
                    ...
                    foreach ($dates as $date) {
                        ...
                        if (($sJ !== '' && $sJ !== 'ENTRANT') || ($sN !== '' && $sN !== 'ENTRANT')) {
                            $assigned_days++;
                    ...
                    if ($scObj) {
                        ...
                        $prorata_base = (int) round($base_old * ($active_days_old / $divisor)) + (int) round($base_new * ($active_days_new / $divisor));
                        ...
                        foreach ($dates as $date) {
                            ...
                            $is_deduct_j = ($sJ === 'A' || in_array($sJ, ['ABANDON', 'DEMISSION', 'SORTANT', 'MAP', 'P'])); // ENTRANT exclu
                            $is_deduct_n = ($sN === 'A' || in_array($sN, ['ABANDON', 'DEMISSION', 'SORTANT', 'MAP', 'P'])); // ENTRANT exclu
                        ...
                        $deductions = (int) round($deduction_days_old * ($base_old / $divisor)) + (int) round($deduction_days_new * ($base_new / $divisor));
                        ...
                        foreach ($sp_details as $spd) {
                            if (isset($spd['replacedAgentBase']) && $spd['replacedAgentBase'] !== null) {
                                ...
                                if ($replaced_base > $base_used_for_deductions) {
                                    // Scénario A : Poste supérieur -> Gagne la différence (bonus)
                                    $gains += (int) round(($replaced_base - $base_used_for_deductions) / $divisor);
                                } else {
                                    // Scénario B : Poste inférieur -> S'adapte au taux remplacé
                                    $gains += (int) round($replaced_base / $divisor);
                                }
                                ...
                                // Scénario C : Aucun remplacement
                                $gains += (int) round($base_used_for_deductions / $divisor);
                            }
                        }

                        // Ajouter les jours de présence normale effectués hors du planning Temps Partiel (au taux journalier standard 100%, sans bonus HS)
