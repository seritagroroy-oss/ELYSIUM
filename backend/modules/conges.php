...
switch ($action) {
    ...
    case 'get_my_leave_balances':
        ...
        if (!$balance) {
            // CrÃ©er le solde par dÃ©faut s'il n'existe pas encore
    ...
    case 'submit_leave_request':
        ...
        echo json_encode(['success' => true, 'message' => 'Demande soumise avec succÃ¨s']);
        break;
    ...
    case 'process_leave_request':
        ...
        if (!$req || $req['status'] !== 'pending') {
            echo json_encode(['success' => false, 'message' => 'Demande introuvable ou dÃ©jÃ  traitÃ©e']);
            break;
        }
        ...
        echo json_encode(['success' => true, 'message' => 'Demande traitÃ©e']);
        break;
    ...
    case 'upload_leave_attachment':
        ...
        if (!in_array($ext, ['pdf', 'png', 'jpg', 'jpeg'])) {
            echo json_encode(['success' => false, 'message' => 'Format non autorisÃ©. Seulement PDF ou Image.']);
            break;
        }
    ...
    case 'admin_add_leave_request':
        // CÃ´tÃ© Manager : Ajout manuel d'un congÃ© sans passer par le portail agent
        $agent_id = $data['agent_id'] ?? '';
        $leave_type_id = $data['leave_type_id'] ?? 0;
        $start_date = $data['start_date'] ?? '';
        $end_date = $data['end_date'] ?? '';
        ...
        echo json_encode(['success' => true, 'message' => 'CongÃ© ajoutÃ© et validÃ© manuellement']);
        break;
    ...
    case 'add_permission':
        ...
        echo json_encode(['success' => true, 'message' => 'Permission enregistrÃ©e avec succÃ¨s']);
        break;
    ...
    case 'delete_permission':
        ...
        echo json_encode(['success' => true, 'message' => 'Permission supprimÃ©e']);
        break;
    ...
    case 'add_contract':
        ...
        if (!$agent_id || !$start_date) {
            echo json_encode(['success' => false, 'message' => 'Agent et date de dÃ©but requis']);
            break;
        }
        ...
        echo json_encode(['success' => true, 'message' => 'Contrat crÃ©Ã© avec succÃ¨s']);
        break;

    case 'update_contract_status':
        $id     = $data['id'] ?? '';
        $status = $data['status'] ?? '';
        if (!$id || !$status) { echo json_encode(['success' => false, 'message' => 'DonnÃ©es manquantes']); break; }
        $db = getDb();
    ...
    // ==========================================
    // MODULE REGISTRE GÃ‰NÃ‰RAL DU PERSONNEL
    // ==========================================
    ...
    case 'get_personnel_registry':
        ...
        foreach ($agents_raw as $ag) {
            // VÃ©rifier abandon dans attendance
            ...
            // VÃ©rifier prÃ©sence dans les 3 derniers mois
            ...
            if (!empty($ag['exit_reason'])) {
                $status = (stripos($ag['exit_reason'], 'Abandon') !== false || stripos($ag['exit_reason'], 'DÃ©mission') !== false) ? 'abandoned' : 'archived';
                $exit_reason = $ag['exit_reason'];
                // exit_date est dÃ©jÃ  dans $ag['exit_date']
            } elseif ($hasAbandon) {
                $status = 'abandoned';
                $exit_reason = 'Abandon de poste';
                // RÃ©cupÃ©rer la date du dernier enregistrement abandon
                if (!$ag['exit_date']) {
            ...
            if (!empty($ag['subsite_name'])) $service_display .= ' â€” ' . $ag['subsite_name'];
        ...
        // --- 2. RÃ©cupÃ©rer tous les UTILISATEURS (admins, contrÃ´leurs) ---
        ...
        foreach ($users_raw as $u) {
            $status = 'active'; // Les users admins sont toujours actifs sauf dÃ©sactivation manuelle
            $registry[] = [
                'id'          => 'usr_' . $u['id'],
                'source'      => 'user',
                'name'        => $u['name'] ?? $u['email'],
