...
switch ($action) {
    case 'archive_payroll':
        ...
        // Mettre Ã  jour le snapshot utilisÃ© par get_salaries pour garantir la cohÃ©rence
    ...
    case 'update_user_photo':
        ...
        echo json_encode(['success' => true, 'message' => 'Photo mise Ã  jour']);
        break;
    ...
    case 'update_user_permissions':
        ...
        echo json_encode(['success' => true, 'message' => 'Permissions mises Ã  jour']);
        break;
    ...
    case 'pointage_gps':
        ...
        echo json_encode(['success' => true, 'message' => 'Pointage GPS enregistrÃ© (Simulation)']);
        break;
    ...
    case 'validate_qr':
        ...
        if ($token === $expected || $token === $expectedPrev) {
            echo json_encode(['success' => true, 'message' => 'Pointage validÃ© par QR Code']);
        } else {
    ...
    case 'get_agents_for_admin':
        ...
        $sqlite = getDb();
        $stmt  = $sqlite->prepare("SELECT id, name, matricule, service_id FROM agents WHERE archived_period IS NULL ORDER BY name AND company_id = ?");
        $stmt->execute();
    ...
    case 'close_payroll_fluctuation':
        ...
        $reclamations_total = $data['reclamations_total'] ?? 0;
        $supplementaires_total = $data['supplementaires_total'] ?? 0;
        $absences_total = $data['absences_total'] ?? 0;
        $ponctions_total = $data['ponctions_total'] ?? 0;
        ...
        try { $sqlite->exec("ALTER TABLE fluctuation_history ADD COLUMN supplementaires_total REAL DEFAULT 0"); } catch (Exception $e) {}
        try { $sqlite->exec("ALTER TABLE fluctuation_history ADD COLUMN absences_total REAL DEFAULT 0"); } catch (Exception $e) {}
        try { $sqlite->exec("ALTER TABLE fluctuation_history ADD COLUMN unbilled_sites_count INTEGER DEFAULT 0"); } catch (Exception $e) {}
        try { $sqlite->exec("ALTER TABLE fluctuation_history ADD COLUMN unbilled_salary_total REAL DEFAULT 0"); } catch (Exception $e) {}
        ...
        $unbilled_sites_count = $data['unbilled_sites_count'] ?? 0;
        $unbilled_salary_total = $data['unbilled_salary_total'] ?? 0;

        $stmt = $sqlite->prepare('REPLACE INTO fluctuation_history (company_id, period, chiffre_affaire, ms_admin, ms_agents, admin_count, agents_count, reclamations_total, supplementaires_total, absences_total, ponctions_total, unbilled_sites_count, unbilled_salary_total, closed_at, closed_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
        $stmt->execute([$companyId, $period, $ca, $ms_admin, $ms_agents, $admin_count, $agents_count, $reclamations_total, $supplementaires_total, $absences_total, $ponctions_total, $unbilled_sites_count, $unbilled_salary_total, $closed_at, $closed_by]);
        
        // Mettre Ã  jour le statut Ã  closed
        setServiceDataSql($companyId, 'fluctuation_status_' . $period, 'closed');
        ...
        foreach ($allAgents as $agent) {
            ...
            $site_name = $agent['site_name'] ?? 'Non affectÃ©';
    ...
    case 'save_site_revenue':
        echo json_encode(['success' => true, 'message' => 'Contrat sauvegardÃ©']);
        break;
        
    case 'save_manual_adjustment':
        echo json_encode(['success' => true, 'message' => 'Ajustement sauvegardÃ©']);
        break;
    ...
    case 'send_reclamation_feedback':
        ...
        // Mettre Ã  jour le dernier feedback pour dÃ©clencher la pop-up chez le publiant
    ...
    case 'add_reclamation':
        ...
        // On ne rÃ©initialise les avis que s'il s'agit d'une nouvelle soumission (ou on laisse le frontend dÃ©cider)
        if (!$isUpdate) {
    ...
    case 'get_agent_absences':
        ...
        $stmt  = $sqlite->prepare("SELECT date, status FROM attendance WHERE agent_id = ? AND period = ? AND company_id = ? AND status IN ('A', 'ABANDON', 'DEMISSION') ORDER BY date ASC");
        $stmt->execute([$agent_id, $att_period, resolveCurrentCompanyIdSql()]);
        $dates = [];
    ...
    case 'update_reclamation_status':
        ...
        if (updateReclamationStatus($id, $updates)) {
            ...
            echo json_encode(['success' => false, 'message' => 'RÃ©clamation non trouvÃ©e']);
        }
        break;
    ...
    case 'get_fluctuation_archives':
        ...
        try {
            ...
            // DÃ©terminer la pÃ©riode courante (la plus rÃ©cente dans published_periods)
            $published = getServiceDataSql($companyId, 'published_periods', []);
            if (!empty($published)) {
                usort($published, function($a, $b) {
                    return strcmp($b, $a);
                });
                $current_period = $published[0];
            } else {
                $current_period = date('Y-m');
            }

            // Exclure la pÃ©riode courante active de la liste des archives
            $archives = array_filter($archives, function($arch) use ($current_period) {
                return $arch['period'] !== $current_period;
            });
    ...
    case 'get_fluctuation_analytics':
        ...
        if ($status !== 'closed') {
            ...
            foreach ($salaries as $agent) {
                ...
                if (isset($agent['deductions'])) {
                    $live_absences_total += (float)$agent['deductions'];
                }
            }
        }
        ...
        try {
            ...
            try { $sqlite->exec("ALTER TABLE fluctuation_history ADD COLUMN reclamations_total REAL DEFAULT 0"); } catch (Exception $e) {}
            try { $sqlite->exec("ALTER TABLE fluctuation_history ADD COLUMN absences_total REAL DEFAULT 0"); } catch (Exception $e) {}
            try { $sqlite->exec("ALTER TABLE fluctuation_history ADD COLUMN ponctions_total REAL DEFAULT 0"); } catch (Exception $e) {}
            try { $sqlite->exec("ALTER TABLE fluctuation_history ADD COLUMN unbilled_sites_count INTEGER DEFAULT 0"); } catch (Exception $e) {}
            try { $sqlite->exec("ALTER TABLE fluctuation_history ADD COLUMN unbilled_salary_total REAL DEFAULT 0"); } catch (Exception $e) {}
            $sqlite->exec('CREATE TABLE IF NOT EXISTS fluctuation_history_sites (company_id TEXT, period TEXT, site_name TEXT, contract_revenue REAL, total_cost REAL, net_margin REAL, is_alert INTEGER, PRIMARY KEY(company_id, period, site_name))');
        } catch (Exception $e) {}
        ...
        if ($status === 'closed') {
            $archive_id = 'payroll_' . $period;
            $stmtArch = $sqlite->prepare("SELECT data FROM archives WHERE id = ? AND company_id = ?");
            $stmtArch->execute([$archive_id, $companyId]);
            $arch_json = $stmtArch->fetchColumn();
            if ($arch_json) {
                $arch_data = json_decode($arch_json, true);
                if (isset($arch_data['salaries']) && is_array($arch_data['salaries'])) {
                    $all_salaries = $arch_data['salaries'];
                }
            }
        }
        ...
        if (empty($all_salaries)) {
            if (!isset($salaries)) {
                require_once __DIR__ . '/../core/functions.php';
                $serviceKey = resolveCurrentServiceKeySql();
                $all_salaries = generateSalariesData($sqlite, $period, $companyId, 'company_id', $companyId, $serviceKey);
            } else {
        ...
        if (!empty($all_salaries)) {
            ...
            $admin_count_real = 0; $agents_count_real = 0;
            $absences_total_real = 0;

            // Compute unbilled metrics
            $stmtUnbilled = $sqlite->prepare("SELECT id, name FROM sites WHERE company_id = ? AND is_billed = 0");
            $stmtUnbilled->execute([$companyId]);
            $unbilledSitesData = $stmtUnbilled->fetchAll(PDO::FETCH_ASSOC);
            $unbilledSiteIds = array_column($unbilledSitesData, 'id');
            $unbilledSiteNames = array_column($unbilledSitesData, 'name');
            $live_unbilled_sites_count = count($unbilledSitesData);
            $live_unbilled_salary_total = 0;

            foreach ($all_salaries as $sal) {
                $net = (float)($sal['net_a_payer'] ?? $sal['net'] ?? $sal['total'] ?? 0);
                if (isset($sal['deductions'])) {
                    $absences_total_real += (float)$sal['deductions'];
                }
                
                $salSiteId = $sal['site_id'] ?? '';
                $salSiteName = $sal['site'] ?? '';
                if (in_array($salSiteId, $unbilledSiteIds) || in_array($salSiteName, $unbilledSiteNames)) {
                    $live_unbilled_salary_total += $net;
                }
            ...
            if (!$snapshot) {
                $snapshot = ['ms_admin' => $ms_admin_real, 'ms_agents' => $ms_agents_real, 'chiffre_affaire' => 0, 'admin_count' => $admin_count_real, 'agents_count' => $agents_count_real, 'reclamations_total' => 0, 'supplementaires_total' => 0, 'absences_total' => $absences_total_real, 'ponctions_total' => 0, 'unbilled_sites_count' => $live_unbilled_sites_count, 'unbilled_salary_total' => $live_unbilled_salary_total];
            }
        } else {
            $stmtUnbilled = $sqlite->prepare("SELECT COUNT(*) FROM sites WHERE company_id = ? AND is_billed = 0");
            $stmtUnbilled->execute([$companyId]);
            $live_unbilled_sites_count = (int)$stmtUnbilled->fetchColumn();
            $live_unbilled_salary_total = 0;
        }

        if (!$snapshot) {
            $stmtBilledLive = $sqlite->prepare("SELECT COUNT(*) FROM sites WHERE company_id = ? AND (is_billed = 1 OR is_billed IS NULL) AND id NOT IN ('site_extras', 'site_releves', 'site_administration')");
            $stmtBilledLive->execute([$companyId]);
            $live_billed_sites_count = (int)$stmtBilledLive->fetchColumn();

            echo json_encode(['success' => true, 'snapshot_exists' => false, 'chiffre_affaire' => 0, 'ms_admin' => 0, 'ms_agents' => 0, 'admin_count' => 0, 'agents_count' => 0, 'reclamations_total' => $live_reclamations_total, 'supplementaires_total' => $live_supplementaires_total, 'absences_total' => $live_absences_total, 'ponctions_total' => $live_ponctions_total, 'billed_sites_count' => $live_billed_sites_count, 'unbilled_sites_count' => $live_unbilled_sites_count, 'unbilled_salary_total' => $live_unbilled_salary_total, 'sites_rentability' => [], 'company_metrics' => ['total_cost' => 0], 'sites_analysis' => null]);
            break;
        }

        // If snapshot exists, ensure unbilled stats are included, fallback to live computation if missing in snapshot
        if (!isset($snapshot['unbilled_sites_count'])) {
            $snapshot['unbilled_sites_count'] = $live_unbilled_sites_count ?? 0;
        }
        if (!isset($snapshot['unbilled_salary_total'])) {
            $snapshot['unbilled_salary_total'] = $live_unbilled_salary_total ?? 0;
        }
        ...
        foreach ($sites_rentability as &$sr) {
            $sr['is_alert'] = (bool)($sr['is_alert'] ?? false);
            $total_cost += (float)($sr['total_cost'] ?? 0);
        }
        unset($sr);
        ...
        foreach ($prevSiteNames as $name => $ps) {
            if ($name !== '' && !isset($currSiteNames[$name])) { $lost_sites[] = ['name' => $name, 'value' => (float)($ps['contract_revenue'] ?? 0)]; $lost_value += (float)($ps['contract_revenue'] ?? 0); }
        }
        $gained_sites = []; $gained_value = 0;
        foreach ($currSiteNames as $name => $cs) {
            if ($name !== '' && !isset($prevSiteNames[$name])) { $gained_sites[] = ['name' => $name, 'value' => (float)($cs['contract_revenue'] ?? 0)]; $gained_value += (float)($cs['contract_revenue'] ?? 0); }
        }
        
        $sites_analysis = ['prev_period' => count($prevSites) > 0 ? $prevPeriod : 'donnÃ©es actuelles', 'curr_count' => count($sites_rentability), 'prev_count' => count($prevSites), 'lost_sites' => $lost_sites, 'lost_value' => $lost_value, 'gained_sites' => $gained_sites, 'gained_value' => $gained_value, 'conge_agents' => [], 'conge_value' => 0];

        echo json_encode([
            'success' => true, 
            'snapshot_exists' => true, 
            'chiffre_affaire' => (float)($snapshot['chiffre_affaire'] ?? 0), 
            'ms_admin' => (float)($snapshot['ms_admin'] ?? 0), 
            'ms_agents' => (float)($snapshot['ms_agents'] ?? 0),
            'admin_count' => (int)($snapshot['admin_count'] ?? 0),
            'agents_count' => (int)($snapshot['agents_count'] ?? 0),
            'reclamations_total' => (float)($snapshot['reclamations_total'] ?? 0),
            'supplementaires_total' => (float)($snapshot['supplementaires_total'] ?? 0),
            'absences_total' => (float)($snapshot['absences_total'] ?? $live_absences_total),
            'ponctions_total' => (float)($snapshot['ponctions_total'] ?? $live_ponctions_total),
            'billed_sites_count' => count($sites_rentability),
            'unbilled_sites_count' => (int)($snapshot['unbilled_sites_count'] ?? 0),
            'unbilled_salary_total' => (float)($snapshot['unbilled_salary_total'] ?? 0),
    ...
    case 'get_fluctuation_trends':
        ...
        try {
            ...
            if ($currentSnap) {
                ...
                $ca_row = $ca_stmt->fetch(PDO::FETCH_ASSOC);
                $ca = $ca_row ? (float)($ca_row['total_budget'] ?? 0) : 0;
                ...
                $ms_agents = $ms_row ? (float)($ms_row['total_salary'] ?? 0) : 0;
