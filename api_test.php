...
function hasPermission($permission)
{
    ...
    // FORCER l'accès aux fluctuations pour la comptabilité, même s'ils ne se sont pas déconnectés
    if ($permission === 'fluctuations' && strpos($_SESSION['user_id'] ?? '', 'comptabilite') !== false) {
        return true;
    }
...
$permissionByAction = [
    ...
    'get_salaries' => 'fluctuations',
    'get_all_agents' => 'fluctuations',
    'get_fluctuation_analytics' => 'fluctuations',
    'save_manual_adjustment' => 'fluctuations',
    'delete_manual_adjustment' => 'fluctuations',
    'save_site_revenue' => 'fluctuations'
];
...
switch ($action) {
    ...
    case 'jarvisse_chat':
        ...
        if ($service_id) {
            ...
            $salary_config = getServiceDataSql($service_id, 'salary_config', []);
            if (!is_array($salary_config)) $salary_config = [];
            $masseSalariale = 0;
            if (!empty($salary_config)) {
                ...
                foreach ($agentRows as $ag) {
                    $funcId = $ag['function_id'] ?? '';
                    $baseSalary = 0;
                    foreach ($salary_config as $sc) {
                        if (($sc['id'] ?? '') === $funcId || ($sc['function_id'] ?? '') === $funcId) {
                            $baseSalary = (int)($sc['base_salary'] ?? $sc['salary'] ?? 0);
                            break;
                        }
                    }
    ...
    case 'get_analytics':
        ...
        // Salary config
        $salary_config_raw = getServiceDataSql($service_id, 'salary_config', []);
        if (!is_array($salary_config_raw)) $salary_config_raw = [];
    ...
    case 'delete_site':
        ...
        if (in_array($site_id, ['site_extras', 'site_releves', 'site_administration'])) {
            echo json_encode(['success' => false, 'message' => 'Impossible de supprimer un site système']);
            break;
        }

        // SQLite cleanup
        $sqlite->prepare('DELETE FROM agents WHERE site_id = ? AND service_id = ?')->execute([$site_id, $serviceKey]);
        $sqlite->prepare('DELETE FROM subsites WHERE site_id = ? AND service_id = ?')->execute([$site_id, $serviceKey]);
        $sqlite->prepare('DELETE FROM sites WHERE id = ? AND service_id = ?')->execute([$site_id, $serviceKey]);
    ...
    case 'add_agent':
        ...
        $stmtAgent = $sqlite->prepare('INSERT INTO agents (id, name, function, shift_type, has_sp, hire_date, recruitment_cost, subsite_id, service_id, company_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
        $stmtAgent->execute([$new_agent_id, $name, $function, $shift_type, 0, date('Y-m-d'), 45000, $subsite_id, $serviceKey, $company_id]);
...
case 'get_settings':
    ...
    case 'update_salary_config':
        requireWritePermission('company_config');
        $serviceKey = $_SESSION['service_id'] ?? null;
        $companyId = $_SESSION['company_id'] ?? 'comp_default_1';
        $cfg = $data['config'] ?? [];
    ...
    case 'save_functions':
        ...
        // 1. Lire les anciennes fonctions
        $oldFuncs = getServiceDataSql($company_key, 'functions', []);
        
        // 2. Trouver les fonctions supprimées
        $newIds = array_map(function($f) { return $f['id']; }, $funcs);
        $deletedFuncs = [];
        foreach ($oldFuncs as $old) {
            if (!in_array($old['id'], $newIds)) {
                $deletedFuncs[] = $old;
            }
        }

        // 3. Mettre à jour les agents et notifier
        if (!empty($deletedFuncs)) {
            $sqlite = getDb();
            $deletedIds = array_map(function($f) { return $f['id']; }, $deletedFuncs);
            $placeholders = implode(',', array_fill(0, count($deletedIds), '?'));
            
            // On met la fonction à vide
            $stmt = $sqlite->prepare("UPDATE agents SET function = '' WHERE company_id = ? AND function IN ($placeholders)");
            $params = array_merge([$company_id], $deletedIds);
            $stmt->execute($params);

            // 4. Envoyer la notification (Historique et Notifications)
            $history = getServiceDataSql($company_key, 'feedback_history', []);
            $period = date('Y-m');
            
            foreach ($deletedFuncs as $df) {
                $feedbackData = [
                    'period' => $period,
                    'service_name' => $_SESSION['user_service'] ?? 'Comptabilité',
            ...
            setServiceDataSql($company_key, 'feedback_history', $history);
            setServiceDataSql($company_key, 'latest_function_deleted', $feedbackData);
        }
    ...
    case 'get_salaries':
        ...
        $salary_config_raw = getServiceDataSql($serviceKey, 'salary_config', []);
        $functions_raw = getServiceDataSql($serviceKey, 'functions', []);

        // Charger la prime par site depuis site_contracts
        $stmtPrimes = $sqlite->prepare("SELECT site_name, prime_site FROM site_contracts WHERE company_id = ?");
        $stmtPrimes->execute([$companyKey]);
        $site_primes_map = [];
        while ($pr = $stmtPrimes->fetch()) {
            $site_primes_map[$pr['site_name']] = (int)($pr['prime_site'] ?? 0);
        }
        ...
        foreach ($sites_rows as $site) {
            ...
            foreach ($subsites_rows as $sub) {
                ...
                foreach ($agents_rows as $agent) {
                    ...
                    $prime_site = (int)($site_primes_map[$site['name']] ?? 0);
                    $total = $base - $deductions + $gains + $prime_site;
    ...
    case 'update_agent_salary':
        ...
        $sqlite = getDb();
        $sqlite->prepare("UPDATE agents SET base_salary = ? WHERE id = ?")->execute([$salary, $agent_id]);
    ...
    case 'get_payroll_loans':
        $db = getScopedData($serviceKey);
        $loans = $db['payroll_loans'] ?? [];
        echo json_encode(['success' => true, 'loans' => $loans]);
        break;

    case 'save_payroll_loans':
        if (($_SESSION['user_role'] ?? '') != 'admin' && strpos(strtolower($_SESSION['user_service'] ?? ''), 'compta') === false && strpos(strtolower($_SESSION['user_service'] ?? ''), 'rh') === false) {
            echo json_encode(['success' => false, 'message' => 'Accès refusé']);
            break;
        }
        $db = getScopedData($serviceKey);
        $db['payroll_loans'] = $data['loans'] ?? [];
        saveScopedData($db, $serviceKey);
        echo json_encode(['success' => true]);
        break;
    ...
    case 'publish_period':
        ...
        // Save to company_id instead of service_id to allow cross-service sharing
        $existing = getServiceDataSql($companyKey, 'published_periods', []);
        if (!in_array($period, $existing)) {
            $existing[] = $period;
            setServiceDataSql($companyKey, 'published_periods', $existing);
        }
    ...
    case 'unpublish_period':
        ...
        setServiceDataSql($companyKey, 'latest_pub_feedback', null);
        setServiceDataSql($companyKey, 'latest_reclamation_feedback', null);
        setServiceDataSql('company::' . $companyKey, 'latest_function_deleted', null);
    ...
    case 'send_pub_feedback':
        ...
        setServiceDataSql($companyKey, 'latest_pub_feedback', $feedbackData);
    ...
    case 'get_latest_feedback':
        ...
        // 1. Function deleted feedback
        $funcDb = getServiceDataSql($company_key, 'latest_function_deleted', null);
        if (!$funcDb) {
            $funcDb = getServiceDataSql($companyKey, 'latest_function_deleted', null);
        }
        
        // 2. Publication/pointage feedback
        $pubDb = getServiceDataSql($companyKey, 'latest_pub_feedback', null);
        if (!$pubDb) {
            // Check legacy
            $legacy = getServiceDataSql($companyKey, 'latest_feedback', null);
            if ($legacy && (($legacy['type'] ?? '') === 'accuse' || ($legacy['type'] ?? '') === 'consulter')) {
                $pubDb = $legacy;
            }
        }
        
        // 3. Reclamation feedback
        $recDb = getServiceDataSql($companyKey, 'latest_reclamation_feedback', null);
        if (!$recDb) {
            // Check legacy
            $legacy = getServiceDataSql($companyKey, 'latest_feedback', null);
            if ($legacy && strpos($legacy['type'] ?? '', 'reclamation_') === 0) {
                $recDb = $legacy;
            }
        }
        ...
        echo json_encode([
            'success' => true,
            'function_deleted' => $funcDb,
            'pub_feedback' => $pubDb,
            'reclamation_feedback' => $recDb
        ]);
        break;
    ...
    case 'save_salary_grid':
        if (!hasPermission('company_config') && !hasPermission('salaries')) {
            echo json_encode(['success' => false, 'message' => 'Accès refusé']);
            break;
        }
        $companyId = $_SESSION['company_id'] ?? 'comp_default_1';
    ...
    case 'save_site_contracts':
        try {
            if (!hasPermission('company_config') && !hasPermission('salaries')) {
                echo json_encode(['success' => false, 'message' => 'Accès refusé. Vous devez avoir la permission de configurer l\'entreprise ou les salaires.']);
                break;
            }
            $companyId = $_SESSION['company_id'] ?? 'comp_default_1';
            $sqlite = getDb();
            $stmt = $sqlite->prepare("INSERT INTO site_contracts (company_id, site_name, budget_mensuel, charges_percent, frais_fixes, prime_site) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(company_id, site_name) DO UPDATE SET budget_mensuel=excluded.budget_mensuel, charges_percent=excluded.charges_percent, frais_fixes=excluded.frais_fixes, prime_site=excluded.prime_site");
            $site_name = $data['site_name'] ?? '';
            ...
            $stmt->execute([$companyId, $site_name, $budget, $charges, $frais, $prime]);
            echo json_encode(['success' => true]);
        } catch (Throwable $e) {
            file_put_contents(__DIR__ . '/scratch/site_error.log', $e->getMessage() . "\n" . $e->getTraceAsString());
            echo json_encode(['success' => false, 'message' => 'Erreur serveur interceptée']);
        }
        break;
    ...
    case 'get_compta_data':
        ...
        $contracts = $sqlite->prepare("SELECT site_name, budget_mensuel, charges_percent, frais_fixes, prime_site FROM site_contracts WHERE company_id=?");
    ...
    case 'send_reclamation_feedback':
        ...
        setServiceDataSql($companyKey, 'latest_reclamation_feedback', $feedbackData);
