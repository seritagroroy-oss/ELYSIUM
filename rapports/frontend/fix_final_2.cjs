...
console.log("Fixing DashboardTable syntax...");
const tablePath = path.join(__dirname, 'src/components/tables/DashboardTable.jsx');
let tCode = fs.readFileSync(tablePath, 'utf8');

// DashboardTable was extracted but missed the })()} closure. Let's fix it safely.
// We look for the last "});" and replace it with "}); })()}" before the "</>".
tCode = tCode.replace(/}\);\s*<\/>\s*\);\s*}\s*$/, '});\n          })()}\n    </>\n  );\n}');
...
// Very safe regex to remove old transferModal
...
// Inject new modals rendering securely at the end of the file
...
// Now re-read table to apply transfer logic
tCode = fs.readFileSync(tablePath, 'utf8');
...
// Revert do_extract.cjs back to normal to prevent Dashboard.jsx from breaking
let extractCode = fs.readFileSync(path.join(__dirname, 'do_extract.cjs'), 'utf8');
extractCode = extractCode.replace('endIndex = i + 1;', 'endIndex = i;');
fs.writeFileSync(path.join(__dirname, 'do_extract.cjs'), extractCode, 'utf8');
