...
import { ShieldAlert, Shield, UserPlus, Calendar, DollarSign, Archive, Settings, LogOut, Clock, Loader2, Sparkles, Menu, X, CheckCircle, Home as HomeIcon, ReceiptText, TrendingUp, MessageSquare, Camera, Bell, Search, BarChart3, Bot, FileWarning, Fingerprint, Contact, Plane, Briefcase, Database, Users, Printer, FileText, MapPin, PlusCircle, Package, MessageSquareWarning, MessageCircle, AlertTriangle, Inbox, Building2, Sun, Moon, User, Mail, Phone, Lock, Monitor, Eye, EyeOff } from 'lucide-react';
import './App.css';
...
const GestionFlotte          = lazy(() => import('./components/GestionFlotte'));
...
const AlertesSecurite        = lazy(() => import('./components/AlertesSecurite'));
...
const PCCCTV                 = lazy(() => import('./components/PCCCTV'));
const PCDispatch             = lazy(() => import('./components/PCDispatch'));
const PCComms                = lazy(() => import('./components/PCComms'));
const PCMainCourante         = lazy(() => import('./components/PCMainCourante'));
const PCTracking             = lazy(() => import('./components/PCTracking'));
...
const CtrlMessagerie         = lazy(() => import('./components/CtrlMessagerie'));
const CtrlCarnet             = lazy(() => import('./components/CtrlCarnet'));
const CtrlTracking           = lazy(() => import('./components/CtrlTracking'));
const CtrlDispatch           = lazy(() => import('./components/CtrlDispatch'));
const CtrlFlotte             = lazy(() => import('./components/CtrlFlotte'));
      ...
      try {
        ...
        if (user.role === 'admin' || user.role === 'super_admin') {
          ...
          if (resetRes?.success && resetRes.count > 0) {
            // Find if there are NEW pending resets not seen before
    ...
    if (!user) {
      ...
      if (user.workspace_type === 'AGENT' && !user.is_impersonated) {
        setView(prev => {
          ...
          return (prev === 'login' || prev === 'register' || prev === 'welcome') ? 'kiosk' : prev;
        });
      } else {
        setView(prev => {
          const stored = localStorage.getItem('pontage_active_view');
