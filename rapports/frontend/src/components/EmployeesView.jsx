...
import { 
  ...
  Save, Heart, GraduationCap, FileText, UploadCloud, Globe, CheckCircle, Badge, MessageSquare, FolderOpen, Download, Settings, AlertTriangle, Printer
} from 'lucide-react';
...
export default function EmployeesView() {
  ...
  const [printDocType, setPrintDocType] = useState(null); // 'attestation' | 'certificat' | null
  ...
  const isHR = user?.role === 'admin' || user?.role === 'super_admin' || user?.permissions?.includes('rh') || user?.service === 'RH';

  const expiringContracts = agents.filter(a => {
    if (!a.profile_data?.contract_end) return false;
    const endDate = new Date(a.profile_data.contract_end);
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', background: 'rgba(255,255,255,0.05)', padding: '10px', borderRadius: '8px' }}>
            ...
            <div style={{ flex: 1, overflow: 'hidden', display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <button type="button" onClick={(e) => { 
                ...
                if (w) {
                  const sanitizedUrl = DOMPurify.sanitize(fileData.data);
                  w.document.write('<style>body{margin:0;overflow:hidden;background:#333;}</style><iframe sandbox="allow-scripts allow-same-origin" src="' + sanitizedUrl + '" frameborder="0" style="width:100vw;height:100vh;border:none;"></iframe>');
                  w.document.close();
                } else {
  ...
  if (selectedAgent) {
    ...
    return (
      <div style={{ padding: '24px', maxWidth: '1400px', margin: '0 auto', animation: 'fadeIn 0.3s' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            <div style={{ position: 'relative' }}>
              <button className="btn btn-secondary" onClick={() => setPrintDocType(printDocType ? null : 'menu')} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'rgba(56, 189, 248, 0.1)', color: '#38bdf8', borderColor: 'rgba(56, 189, 248, 0.3)' }}>
                <Printer size={16} /> Générer un document
              </button>
              {printDocType === 'menu' && (
                <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: '8px', background: 'white', borderRadius: '8px', boxShadow: '0 4px 20px rgba(0,0,0,0.5)', zIndex: 100, width: '220px', overflow: 'hidden' }}>
                  <button onClick={() => setPrintDocType('attestation')} style={{ width: '100%', padding: '12px 16px', background: 'transparent', border: 'none', borderBottom: '1px solid #e2e8f0', color: '#1e293b', textAlign: 'left', cursor: 'pointer', fontSize: '0.9rem', fontWeight: 600 }} onMouseEnter={e => e.currentTarget.style.background = '#f1f5f9'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>Attestation de Travail</button>
                  <button onClick={() => setPrintDocType('certificat')} style={{ width: '100%', padding: '12px 16px', background: 'transparent', border: 'none', color: '#1e293b', textAlign: 'left', cursor: 'pointer', fontSize: '0.9rem', fontWeight: 600 }} onMouseEnter={e => e.currentTarget.style.background = '#f1f5f9'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>Certificat de Travail</button>
                </div>
              )}
            </div>
                ...
                <div style={{ animation: 'fadeIn 0.3s' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px' }}>
                    <div>
                      ...
                      <h3 style={{ fontSize: '1.1rem', color: 'var(--a)', marginBottom: '20px', marginTop: '24px' }}>Paie & Comptabilité</h3>
                      {renderField('Numéro Sécurité Sociale (CNPS)', 'num_secu_sociale')}
                      {renderField('Convention Collective', 'convention_collective')}
                      {renderField('Catégorie', 'categorie')}
                      {renderField('Qualification / Emploi', 'qualification')}
                      {renderField('Coefficient', 'coefficient', 'number')}
                      {renderField('Indice', 'indice', 'number')}
                      {renderField('Horaire Mensuel (ex: 173.33)', 'horaire_mensuel', 'number')}
                    </div>
                    <div>
                        ...
                        <div style={{ width: '100%', height: '210mm', display: 'flex', alignItems: 'center', justifyContent: 'center', boxSizing: 'border-box' }}>
                          {isImage ? (
                            ...
                            <div style={{ width: '100%', height: '100%', borderRadius: '4px', overflow: 'hidden', background: 'white' }}>
                               <iframe sandbox="allow-scripts allow-same-origin" src={`${file.data}#view=FitH`} style={{ width: '100%', height: '100%', border: 'none', backgroundColor: 'white' }} title={doc.label} />
                            </div>
                          ) : (
        ...
        {/* MODAL GENERATION DOCUMENTS (ATTESTATION / CERTIFICAT) */}
        {(printDocType === 'attestation' || printDocType === 'certificat') && (
          <div className="dossier-modal-overlay" style={{
            position: 'fixed', inset: 0, background: '#525659',
            display: 'flex', flexDirection: 'column', zIndex: 10000, animation: 'fadeIn 0.2s ease-out'
          }}>
            <div className="dossier-toolbar" style={{ background: '#323639', padding: '12px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #202124', boxShadow: '0 2px 4px rgba(0,0,0,0.2)' }}>
              <h2 style={{ fontSize: '1rem', margin: 0, color: 'white', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 500 }}>
                <Printer size={18}/> {printDocType === 'attestation' ? 'Attestation de Travail' : 'Certificat de Travail'} - {a.name}
              </h2>
              <div style={{ display: 'flex', gap: '16px' }}>
                <button onClick={() => window.print()} style={{ background: 'transparent', color: 'white', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 12px', borderRadius: '4px' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.1)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                  <Download size={18}/> Imprimer / Enregistrer en PDF
                </button>
                <button onClick={() => setPrintDocType(null)} style={{ background: 'transparent', border: 'none', color: '#9aa0a6', cursor: 'pointer', display: 'flex', alignItems: 'center', padding: '6px' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.1)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                  <X size={20} />
                </button>
              </div>
            </div>

            <div className="dossier-scroll-area" style={{ flex: 1, overflowY: 'auto', padding: '20px 0', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div id="dossier-print-container" style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <style>
                  {`
                    ...
                    @media print {
                      body * { visibility: hidden !important; }
                      .dossier-modal-overlay { position: absolute !important; top: 0 !important; left: 0 !important; background: transparent !important; height: auto !important; display: block !important; }
                      .dossier-toolbar { display: none !important; }
                      .dossier-scroll-area { padding: 0 !important; height: auto !important; display: block !important; }
                      #dossier-print-container { visibility: visible !important; display: block !important; position: relative !important; }
                      #dossier-print-container * { visibility: visible !important; }
                      .a4-page { margin: 0 !important; box-shadow: none !important; width: 210mm !important; min-height: 297mm !important; display: block !important; position: relative !important; }
                    }
                  `}
                </style>
                <div className="a4-page" style={{ width: '210mm', minHeight: '297mm', padding: '25mm', margin: '0 auto', background: 'white', boxShadow: '0 4px 10px rgba(0,0,0,0.5)', boxSizing: 'border-box', position: 'relative', color: 'black' }}>
                  {/* Header En-tête */}
                  <div style={{ textAlign: 'center', borderBottom: '2px solid #0f172a', paddingBottom: '16px', marginBottom: '40px' }}>
                    <h1 style={{ margin: 0, fontSize: '1.8rem', color: '#0f172a', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '1px' }}>{printSettings.companyName}</h1>
                    <p style={{ margin: '4px 0 0 0', fontSize: '0.9rem', color: '#475569' }}>Société de Sécurité et de Gardiennage</p>
                  </div>

                  {/* Titre du document */}
                  <div style={{ textAlign: 'center', marginBottom: '50px' }}>
                    <h2 style={{ fontSize: '1.5rem', fontWeight: 'bold', margin: 0, textTransform: 'uppercase', textDecoration: 'underline', textUnderlineOffset: '6px' }}>
                      {printDocType === 'attestation' ? 'ATTESTATION DE TRAVAIL' : 'CERTIFICAT DE TRAVAIL'}
                    </h2>
                  </div>

                  {/* Corps du texte */}
                  <div style={{ fontSize: '1.1rem', lineHeight: '2', textAlign: 'justify', color: '#0f172a' }}>
                    <p>Nous soussignés, la Direction de <strong>{printSettings.companyName}</strong>, certifions par la présente que :</p>
                    
                    <p style={{ marginTop: '30px' }}>
                      M./Mme <strong>{a.name}</strong><br/>
                      {pData.birth_date && pData.birth_place ? `Né(e) le ${pData.birth_date} à ${pData.birth_place}` : ''}<br/>
                      {pData.address ? `Demeurant à : ${pData.address}` : ''}
                    </p>

                    <p style={{ marginTop: '30px' }}>
                      {printDocType === 'attestation' ? 'Est actuellement employé(e)' : 'A été employé(e)'} au sein de notre entreprise en qualité de <strong>{a.function || 'Agent de Sécurité'}</strong>.
                      {pData.contract_start ? ` Son contrat a débuté le ${new Date(pData.contract_start).toLocaleDateString('fr-FR')}.` : ''}
                    </p>
                    
                    {printDocType === 'certificat' && (
                      <p>Il/Elle a quitté notre entreprise {pData.date_sortie ? `le ${new Date(pData.date_sortie).toLocaleDateString('fr-FR')}` : 'à ce jour'}, libre de tout engagement.</p>
                    )}

                    <p style={{ marginTop: '30px' }}>
                      Cette {printDocType === 'attestation' ? 'attestation' : 'présente'} est délivrée à l'intéressé(e) pour servir et valoir ce que de droit.
                    </p>
                  </div>
                  ...
                  <div style={{ marginTop: '80px', display: 'flex', justifyContent: 'flex-end' }}>
                    <div style={{ textAlign: 'center' }}>
                      <p style={{ margin: '0 0 60px 0', fontSize: '1rem' }}>Fait à ........................................, le {new Date().toLocaleDateString('fr-FR')}</p>
                      <p style={{ margin: 0, fontWeight: 'bold' }}>La Direction des Ressources Humaines</p>
                      <p style={{ margin: '4px 0 0 0', fontSize: '0.9rem', color: '#64748b' }}>Cachet et Signature</p>
                    </div>
                  </div>
                </div>
              </div>
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1400px', margin: '0 auto', animation: 'fadeIn 0.3s' }}>
      {isHR && expiringContracts.length > 0 && (
        <div style={{ marginBottom: '24px', padding: '16px 20px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: '12px', display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
          <AlertTriangle size={24} color="#ef4444" style={{ flexShrink: 0 }} />
          <div>
            <h3 style={{ margin: '0 0 4px 0', color: '#ef4444', fontSize: '1.05rem', fontWeight: 600 }}>Attention : Contrats arrivant à échéance ({expiringContracts.length})</h3>
            <p style={{ margin: 0, color: '#fca5a5', fontSize: '0.9rem' }}>
              {expiringContracts.map(a => `${a.name} (le ${new Date(a.profile_data.contract_end).toLocaleDateString()})`).join(', ')}
            </p>
          </div>
        </div>
      )}
