...
export default function Salaries() {
  ...
  const isPastErrorMotif = ["erreur de paie", "erreur de pointage"].includes(newReclamation.motif.toLowerCase());
  ...
  useEffect(() => {
    if (isAutoCalculatedMotif && newReclamation.agent_id) {
      ...
      if (agentData) {
        ...
        if (motif === "justificatif d'absence" && agentData.absence_details) {
          setAbsentDatesList(agentData.absence_details.map(d => d.date || d.start_date));
  ...
  useEffect(() => {
    if ((isAutoCalculatedMotif || isPastErrorMotif) && newReclamation.agent_id && isMontantLocked) {
      ...
      if (agentData) {
        ...
        const montant = Math.round(dailySalary * (Number(newReclamation.jours) || 0));
        setNewReclamation(prev => ({ ...prev, montant }));
      }
    }
  }, [newReclamation.jours, newReclamation.agent_id, newReclamation.motif, salaries, isMontantLocked]);
  ...
  const handleSaveReclamation = async () => {
    ...
    try {
      const res = await apiCall('save_reclamation', { ...newReclamation, period }, 'POST');
      if (res.success) {
  ...
  const calculatePayslip = useCallback((agent) => {
    ...
    const gainsHS = Math.round((agent.sp_count || 0) * baseJournaliere);
    ...
    const motifsRecl = ["justificatif d'absence", "annulation de permission", "mise à pied"];
    ...
    const netAPayer = Math.max(0, salaireBrut - totalRetenuesFiscales - totalDeductionsNettes) + montantReclamations;
          ...
          {activeTab === 'reclamations' && (() => {
            ...
            <div className="glass-panel" style={{ animation: 'slideUp 0.3s ease-out' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', borderBottom: '1px solid var(--border)', paddingBottom: '12px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px', background: 'rgba(0,0,0,0.2)', padding: '6px', borderRadius: '14px', alignItems: 'center' }}>
                  <button 
                    onClick={() => setShowAddReclamationModal(true)}
                    className="btn btn-primary" 
                        ...
                        return recsValidees.map(r => {
                          ...
                          return (
                            <tr key={r.id}>
                              ...
                              <td style={{ textAlign: 'right', color: '#34d399', fontWeight: 'bold' }}>{r.montant_estime ? `${fmt(r.montant_estime)} FCFA` : '-'}</td>
                              <td>
                                {isAbsence ? (
                        ...
                        {journalActuelLoading ? (
                          ...
                          <div className="table-container" style={{ maxHeight: '600px', overflowY: 'auto' }}>
                            ...
                            {journalActuelData.filter(s =>
                              ...
                              return (<tr key={s.id}>
                                <td style={{ fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center' }} onClick={() => setPaymentMethodModal(s)} title="Cliquez pour modifier le mode de paiement">
                                  {s.name}
      ...
      {showAddReclamationModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div className="glass-panel" style={{ width: '100%', maxWidth: '1050px', padding: '40px', background: '#0f172a', border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)' }}>
            <h3 style={{ marginTop: 0, borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '16px', marginBottom: '24px', fontSize: '1.5rem' }}>Ajouter une réclamation</h3>
                  ...
                  {showReclamationSuggestions && newReclamation.agent_name && (
                    <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: '#1e293b', border: '1px solid var(--border)', borderRadius: '8px', maxHeight: '200px', overflowY: 'auto', zIndex: 50, marginTop: '4px', boxShadow: '0 10px 25px rgba(0,0,0,0.5)' }}>
                      {salariesLoading ? (
                        ...
                        <>
                          {salaries.filter(s => (s.name || '').toLowerCase().includes((newReclamation.agent_name || '').toLowerCase())).map(s => (
                            <div 
                              key={s.id}
                              style={{ padding: '10px 14px', cursor: 'pointer', borderBottom: '1px solid rgba(255,255,255,0.05)' }}
                              onMouseDown={(e) => {
                                e.preventDefault();
                                setNewReclamation({...newReclamation, agent_name: s.name, agent_id: s.id});
                          ...
                          {salaries.filter(s => (s.name || '').toLowerCase().includes((newReclamation.agent_name || '').toLowerCase())).length === 0 && (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                <div>
                  ...
                  <input 
                    ...
                    onChange={e => setNewReclamation({...newReclamation, jours: parseFloat(e.target.value) || 0})}
                  />
                </div>
                <div style={{ position: 'relative' }}>
                  ...
                  {showPastCalendar && (() => {
                     ...
                     const agentRecs = reclamations.filter(r => r.agent_id === newReclamation.agent_id);
                     let claimedStrings = [];
                     agentRecs.forEach(r => {
                     ...
                     return (
                       <>
                         ...
                         <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '90%', maxWidth: '400px', background: '#1e293b', border: '1px solid var(--border)', borderRadius: '12px', maxHeight: '80vh', overflowY: 'auto', zIndex: 50, boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)', padding: '20px' }}>
                           ...
                           {availableDates.length > 0 && <div style={{ marginBottom: '10px', color: 'var(--muted)', fontSize: '0.85rem' }}>Dates disponibles (Cycle du 21 au 20) :</div>}
                    ...
                    <>
                      ...
                      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '90%', maxWidth: '400px', background: '#1e293b', border: '1px solid var(--border)', borderRadius: '12px', maxHeight: '80vh', overflowY: 'auto', zIndex: 50, boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)', padding: '20px' }}>
                        {absentDatesList.length === 0 ? (
                          ...
                          // Extraction des dates déjà réclamées pour cet agent
                          const agentRecs = reclamations.filter(r => r.agent_id === newReclamation.agent_id);
                          let claimedStrings = [];
                          agentRecs.forEach(r => {
                            if (r.motif && (r.motif.includes("justificatif") || r.motif.includes("annulation") || r.motif.includes("mise à pied") || r.motif.includes("erreur de paie") || r.motif.includes("erreur de pointage"))) {
                          ...
                          return (
                            <>
                              ...
                              {availableDates.length > 0 && (
                                <div style={{ marginBottom: '10px', color: 'var(--muted)', fontSize: '0.85rem' }}>Dates disponibles :</div>
                              )}
                              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(90px, 1fr))', gap: '8px' }}>
                                {availableDates.map(({ dateStr, formatted }) => {
                                   const isChecked = checkedDates.includes(dateStr);
