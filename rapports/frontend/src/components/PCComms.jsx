          ...
          <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: '12px', padding: '24px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {NETWORK_NODES.map((node) => (
                <div key={node.name || node.ip} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px', background: node.status === 'offline' ? 'rgba(239,68,68,0.05)' : 'rgba(255,255,255,0.02)', border: `1px solid ${node.status === 'offline' ? 'rgba(239,68,68,0.3)' : '#1e293b'}`, borderRadius: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
