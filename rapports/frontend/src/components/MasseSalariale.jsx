...
export default function MasseSalariale({ salaries, period, payrollSettings }) {
  ...
  const processedSalaries = salaries.map(agent => {
    ...
    const its = parseFloat(payrollSettings?.its ?? 0);
    const fdfp = parseFloat(payrollSettings?.fdfp ?? 0);
    const apprentissage = parseFloat(payrollSettings?.taxe_apprentissage ?? 0);
    const accident = parseFloat(payrollSettings?.accidents_travail ?? 0);
    const cmu = parseFloat(payrollSettings?.cmu_amount ?? 0);

    const cnpsBase = Math.min(agent.base, 1647315);
    const cnpsSal = cnpsBase * (parseFloat(payrollSettings?.cnps_salarial ?? 0) / 100);
