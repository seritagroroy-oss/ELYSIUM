...
export default function ReclamationsView() {
  ...
  const renderAdminSection = () => (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '25px' }}>
      <SelectWithOther label="Type d'erreur" field="type_erreur" fieldAutre="type_erreur_autre" options={['Abandon de poste(s)', 'Absence', 'Erreur de pointage(s)', 'Mise à pied', 'Arrêt de pointage', 'Supplementaire', 'Autre']} formData={formData} onChange={handleFieldChange} />
      ...
      <div className="fade-in" style={{ padding: '0 0 40px 0' }}>
        ...
        {(() => {
          ...
          if (awaitingProcessing) {
            ...
            return (
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: '60px', animation: 'slideUp 0.5s ease-out' }}>
                <div className="glass-panel" style={{ padding: '50px', borderRadius: '24px', textAlign: 'center', maxWidth: '620px', width: '100%', border: '1px solid rgba(16,185,129,0.3)', boxShadow: '0 20px 40px rgba(16,185,129,0.1)' }}>
                  <div style={{ display: 'inline-flex', padding: '20px', borderRadius: '50%', background: 'rgba(16,185,129,0.1)', marginBottom: '30px' }}>
                    <CheckCircle size={64} color="#10b981" />
                  </div>
                  <h2 style={{ fontSize: '2rem', color: 'white', marginBottom: '20px' }}>Fiches Transmises · En cours de traitement</h2>
                  <p style={{ color: '#cbd5e1', fontSize: '1.15rem', lineHeight: '1.8', marginBottom: '10px' }}>
                    <strong style={{ color: '#10b981' }}>{pendingForDisplay.length}</strong> fiche(s) de réclamation ont bien été transmises au(x) service(s) :
                  </p>
                  <p style={{ marginBottom: '35px', fontSize: '1.2rem' }}>
                    <strong style={{ color: '#38bdf8' }}>
                      {servicesList.includes('Tous') ? 'Tous les services' : servicesList.join(', ') || '—'}
                    </strong>
                  </p>
                  <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '14px', padding: '20px 25px', color: '#94a3b8', fontSize: '1rem', lineHeight: '1.7' }}>
                    ⏳ Veuillez patienter que le(s) service(s) concerné(s) finalise(nt) le traitement.<br/>
                    Cette page se mettra à jour automatiquement lorsque les fiches seront traitées.
                  </div>
                </div>
              </div>
            );
