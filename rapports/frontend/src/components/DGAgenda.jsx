        ...
        <div className="glass-panel" style={{ padding: '24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '4px' }}>
            {Array.from({ length: totalCells }).map((_, i) => {
              ...
              return (
                <div key={`cell-${i}`} style={{ minHeight: '70px', padding: '6px', borderRadius: '8px', background: isToday ? 'rgba(56,189,248,0.15)' : events.length > 0 ? 'rgba(255,255,255,0.03)' : 'transparent', border: isToday ? '1px solid rgba(56,189,248,0.5)' : '1px solid transparent', opacity: isValid ? 1 : 0.2 }}>
                  {isValid && (
                    <>
                      <div style={{ color: isToday ? '#38bdf8' : 'white', fontWeight: isToday ? 'bold' : 'normal', fontSize: '0.9rem', marginBottom: '4px' }}>{dayNum}</div>
                      {events.map((ev) => (
                        <div key={ev.date + ev.label} style={{ background: `${ev.color}20`, borderLeft: `3px solid ${ev.color}`, padding: '2px 4px', borderRadius: '3px', fontSize: '0.7rem', color: 'white', marginBottom: '2px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                          {ev.icon} {ev.label}
                        </div>
                      ))}
                    </>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div className="glass-panel" style={{ padding: '20px' }}>
            ...
            {upcomingEvents.map((ev, i) => (
              <div key={ev.date + ev.label} style={{ display: 'flex', gap: '12px', padding: '12px 0', borderBottom: i < upcomingEvents.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
