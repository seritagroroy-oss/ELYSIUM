...
import UnbilledSitesModal from './modals/UnbilledSitesModal';

export default function ContratsClientsView({
  sites, unbilledSites, siteContracts, subsiteContracts, configFunctions = [],
  setSites, setUnbilledSites, setSiteContracts, setSubsiteContracts, formatMoney, searchTerm = ''
}) {
  ...
  const refreshSites = async () => {
    const res = await apiCall('get_sites', { scope: 'company', module: 'FACTURATION', is_billed: 1 }, 'GET');
    if (Array.isArray(res)) setSites(res);
    else if (res?.sites) setSites(res.sites);
  };
  ...
  const handleAddSite = async () => {
    ...
    setLoading(true);
    try {
      await apiCall('add_site', { name, location: 'abidjan', module: 'FACTURATION' }, 'POST');
      await refreshSites();
      setNewSiteName('');
  ...
  const handleDeleteSite = (siteId, e) => {
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        ...
        setLoading(true);
        try {
          await apiCall('delete_site', { site_id: siteId }, 'POST');
          await refreshSites();
        } catch (e) { console.error(e); }
        setLoading(false);
      }
  ...
  const handleDeleteSubsite = (subsiteId, e) => {
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        ...
        setLoading(true);
        try {
          await apiCall('delete_subsite', { subsite_id: subsiteId }, 'POST');
          await refreshSites();
        } catch (e) { console.error(e); }
        setLoading(false);
      }
  ...
  const loadArchives = async () => {
    try {
      const res = await apiCall('get_contract_ruptures', {}, 'GET');
      if (res.success) {
        ...
        {!activeSite ? (
          <>
            <div style={{ display: 'flex', gap: '24px', alignItems: 'center' }}>
              ...
              <button onClick={() => setShowUnbilledModal(true)}
                style={{ background: 'rgba(56, 189, 248, 0.1)', color: '#38bdf8', border: '1px solid rgba(56, 189, 248, 0.25)', borderRadius: '9px', padding: '9px 16px', cursor: 'pointer', fontWeight: 600, fontSize: '0.88rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <FileX size={16} /> Clients non facturés
              </button>
            </div>
            <div style={{ display: 'flex', gap: '8px' }}>
