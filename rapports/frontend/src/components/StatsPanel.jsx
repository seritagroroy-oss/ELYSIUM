...
import { TrendingUp, Users, UserCheck, UserX, Clock, RefreshCw, X, Percent, Activity } from 'lucide-react';
...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{
      ...
      {payload.map((p, i) => (
        <p key={p.name || `payload-${i}`} style={{ margin: '2px 0', color: p.color }}>
        ...
        {loading ? (
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 24 }}>
              ...
              <KpiCard icon={Percent}   label="Absentéisme (Période)" value={`${stats?.absenteeism ?? 0}%`} color={COLORS.absent} />
              <KpiCard icon={Activity}  label="Turnover (Période)"    value={`${stats?.turnover ?? 0}%`}    color={COLORS.secondary} />
            </div>
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    <PieChart>
                      <Pie data={pieData} cx="50%" cy="50%" innerRadius={50} outerRadius={80} dataKey="value" paddingAngle={3}>
                        {pieData.map((entry, i) => (
                          <Cell key={entry.name || `cell-${i}`} fill={PIE_COLORS[i]} stroke="transparent" />
                        ))}
                      </Pie>
