...
export default function Dashboard({ isVerificationMode = false, archiveData = null, onBack = null }) {
    ...
    if (forceOverride && overlaps.length > 0) {
      for (const ol of overlaps) {
        ...
        while (oCursor <= oEnd) {
          ...
          shiftCodes.forEach(sc => oUpdates.push({ agent_id: ol.agent_id, date: dk, shift_code: sc, status: '', period: `${pY}-${String(pM).padStart(2, '0')}` }));
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const [res, leavesRes, uiPrefsRes, pubPeriodsRes] = await Promise.all([
        ...
        apiCall('get_published_periods', { scope: 'company', company_id: currentCompanyId, service_id: currentServiceId }, 'GET')
      ]);
  ...
  const loadPublishedPeriods = async () => {
    try {
      const res = await apiCall('get_published_periods', { 
        scope: 'company',
  ...
  const handleCreateAgentFromModal = async (agentData) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        if (agentData.isDebut && agentData.debutDate) {
          if (!res.agent_id) {
            alert('Erreur: agent_id non retourné par le backend lors de la création.');
          } else {
            console.log("Appel de mark_agent_debut avec :", {
              agent_id: res.agent_id,
            ...
            console.log("Résultat de mark_agent_debut :", resDebut);
            if (!resDebut.success) {
          ...
          >
            ...
            <div className="sites-actions-bar">
              ...
              {!isVerificationMode && viewMode === 'current' && !isEmptyMonth && (
                <div className="sites-action-buttons">
                  <button 
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          if (res && res.success) {
                            // Réinitialisation complète - le backend a déjà réinitialisé max_initialized_period
                            setPublishedPeriods([]);
                            localStorage.removeItem('pontage_payroll_statuses');
                            // Réinitialiser le state local au mois courant
                            const todayMonth = new Date().toISOString().slice(0, 7);
                            setMaxInitializedPeriod(todayMonth);
                            setPeriod(todayMonth);
                            localStorage.setItem('pontage_period', todayMonth);
                            alert("✅ Dépublication réussie ! Le système a été réinitialisé au mois courant.");
                          } else {
                            alert("Échec de la dépublication : " + (res ? res.message : "Réponse vide de l'API"));
                          }
                        } catch(e) {
                          alert("Erreur de connexion API : " + e.message);
                          console.error(e);
                        }
                      }}
                      style={{
            ...
            {!isEmptyMonth && (
              <div style={{ flexBasis: '100%', display: 'flex', justifyContent: 'flex-end', marginTop: '4px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', background: 'white', padding: '10px 20px', borderRadius: '30px', border: '1px solid rgba(0,0,0,0.1)', width: '100%', maxWidth: '400px', boxShadow: '0 4px 15px rgba(0,0,0,0.1)' }}>
              ...
              <div className="sites-grid">
                  {sites.filter(site => {
                     ...
                     if (siteSearchTerm && site.name) {
                       if (!site.name.toLowerCase().includes(siteSearchTerm.toLowerCase())) {
                         return false;
                       }
                     }
                     return true;
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                          ...
                          {!['site_extras', 'site_releves', 'site_administration'].includes(site.id) && (
                            <button
                              ...
                              onClick={async (e) => {
                                e.stopPropagation();
                                if(window.confirm("Voulez-vous vraiment supprimer ce site ? Tous les agents et historiques liés seront effacés !")) {
                                  try {
                                    const res = await apiCall('delete_site', { site_id: site.id, motif: 'Suppression depuis dashboard' });
                                    if (res.success) {
                                      const newSites = sites.filter(s => s.id !== site.id);
                                      setSites(newSites);
                                      localStorage.setItem('pontage_sites_cache', JSON.stringify(newSites));
                                      if (activeSiteId === site.id) setActiveSiteId(null);
                                    } else {
  ...
  return (
    <div onClick={() => { setContextMenu(null); setReposMenu(null); }}>
      ...
      {!isZenMode && (
        ...
        functions={activeSiteId === 'site_administration' ? functions.filter(f => f.type === 'admin') : functions.filter(f => f.type !== 'admin')}
        ...
        <div className="locked-period-alert" style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.55)', marginTop: '2px' }}>
              ...
              <span style={{ opacity: 0.7 }}> → Utilisez «&nbsp;Reset Test&nbsp;» pour dépublier si une correction est indispensable.</span>
            </div>
          </div>
        ...
        <div 
          ...
          {!selectedKpiAgent ? (
            ...
            (selectedKpiAgent.attendance || []).forEach(att => {
              ...
              if (att.shift_code === 'S' || att.shift_code === 'SJ' || att.shift_code === 'SN') {
                if (att.status !== 'A' && att.status !== 'R') {
                  overtimes++;
                  if (scObj && att.date < scObj.date) overtimesOld++;
                  else if (scObj && att.date >= scObj.date) overtimesNew++;
                }
            ...
            if (scObj) {
                ...
                gains = Math.round((baseSalaryOld / specialBase) * overtimesOld) + Math.round((baseSalaryNew / specialBase) * overtimesNew);
                ...
                gains = Math.round(dailyRate * overtimes);
...
/>
      ...
      {/* ============ MODAL AVERTISSEMENT CHEVAUCHEMENT ============ */}
      {overlapWarning && (
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }}>
              ...
              {(activeSiteId === 'site_administration' ? functions.filter(f => f.type === 'admin') : functions.filter(f => f.type !== 'admin')).map(f => (
                <button
                  key={f.id}
      ...
      {zoneConfigModalData && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, backdropFilter: 'blur(4px)' }}>
          <div className="glass-panel" style={{ width: '100%', maxWidth: '800px', maxHeight: '85vh', overflowY: 'auto', background: 'linear-gradient(145deg, #1e293b 0%, #0f172a 100%)', border: '1px solid rgba(255,255,255,0.15)', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)' }}>
            ...
            <h4 style={{ marginTop: '24px', marginBottom: '16px', color: 'white', fontSize: '1.2rem', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '8px' }}>Fonctions (Postes)</h4>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '12px', marginBottom: '32px' }}>
              {functions.map(f => {
                 ...
                 return (
                   ...
                   >
                     <div>
                       <strong style={{ display: 'block', color: 'var(--text)', fontSize: '0.95rem' }}>🛡️ {f.fullName || f.id} <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.8rem', fontWeight: 'normal', marginLeft: '4px' }}>({f.id})</span></strong>
                     </div>
                     <div style={{ display: 'flex', alignItems: 'center' }}>
                       <input 
                         type="checkbox" 
            ...
            <button 
              ...
              style={{ marginTop: '10px', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '8px', background: 'rgba(56, 189, 248, 0.1)', border: '1px dashed rgba(56, 189, 248, 0.4)', color: '#38bdf8', marginBottom: '20px' }}
              onClick={(e) => {
                e.preventDefault();
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <button className="btn btn-primary" onClick={async () => {
                 await handleUpdateSubsiteConfig(zoneConfigModalData.id, zoneConfigModalData.costume_enabled === 1, zoneConfigModalData.enabled_functions || []);
