...
$settings_raw = getServiceDataSql($serviceKey, 'settings', ['cycle_start' => 21, 'cycle_end' => 20]);
            ...
            foreach ($subsites_rows as $sub) {
                ...
                foreach ($agents_rows as $agent) {
                    ...
                    $salaries[] = [
                        ...
                        'days_worked' => max(0, ($is_special ? ($real_active + $tp_extra_dates_count) : $active_days) - ($absences + $map_count + $permission_count + ($entrant_sortant_count - $entrant_count))),
                        'att_count' => $att_count,
                        'absences' => $absences,
        ...
        foreach ($salaries as $sal) {
            ...
            $salIsClone = (bool)preg_match('/ag_\d+_/', $sal['id']);
            ...
            foreach ($merged_map as $key => $existing) {
                ...
                $existingIsClone = (bool)preg_match('/ag_\d+_/', $existing['id']);
                ...
                $hasMutationMarker = function($entry) {
                    ...
                    foreach (array_merge($entry['sp_details'] ?? [], $entry['absence_details'] ?? []) as $det) {
                        $txt = json_encode($det);
                        if (strpos($txt, 'MUT') !== false || strpos($txt, 'REL_') !== false || strpos($txt, 'EXT_') !== false || strpos($txt, 'PM|') !== false) {
                            return true;
                        }
                    }
                    return false;
