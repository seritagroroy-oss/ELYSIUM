...
try {
    ...
    // 4. Supprimer l'archive du pointage corrompue
    $arch = $db->prepare("DELETE FROM archives_pointage WHERE period = ?");
    $arch->execute([$periodToUnpublish]);
    echo "Archives pointage supprimées : " . $arch->rowCount() . "<br/>";
    
    // 5. Supprimer la photo figée du module calcul salaire (Masse Salariale / Clôture)
    $fl1 = $db->prepare("DELETE FROM fluctuation_history WHERE period = ?");
    $fl1->execute([$periodToUnpublish]);
    echo "Historique fluctuation global supprimé : " . $fl1->rowCount() . "<br/>";

    $fl2 = $db->prepare("DELETE FROM fluctuation_history_sites WHERE period = ?");
    $fl2->execute([$periodToUnpublish]);
    echo "Historique fluctuation sites supprimé : " . $fl2->rowCount() . "<br/>";

    $arch2 = $db->prepare("DELETE FROM archives WHERE period = ? AND id = ?");
    $arch2->execute([$periodToUnpublish, 'payroll_' . $periodToUnpublish]);
    echo "Archives état de paie supprimées : " . $arch2->rowCount() . "<br/>";

    $st = $db->prepare("DELETE FROM service_data WHERE data_key = ?");
    $st->execute(['fluctuation_status_' . $periodToUnpublish]);
    echo "Statut de clôture supprimé : " . $st->rowCount() . "<br/>";
