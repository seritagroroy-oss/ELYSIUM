        ...
        foreach ($sites_rows as $site) {
            ...
            foreach ($subsites_rows as $sub) {
                ...
                foreach ($agents_rows as $agent) {
                    ...
                    // On détecte si l'agent a été muté depuis un autre site (texte long dans la grille)
                    $has_mutation_in = false;
                    foreach ($dates as $d_check) {
                        $sJ_c = $att_map['J'][$d_check] ?? '';
                        if (is_string($sJ_c) && strlen($sJ_c) > 12 && strpos($sJ_c, 'F_') !== 0 && strpos($sJ_c, 'SORTANT_') !== 0) {
                            if (strpos($sJ_c, ' - ') !== false || strpos($sJ_c, 'MUTÉ VERS') !== false) {
                                $has_mutation_in = true;
                                break;
                            }
                        }
                    }

                    if ($is244872 && $totalRuptureBackend > 0 && !$has_mutation_in) {
                            ...
                            if ($sp_status !== '' && $sp_status !== 'A' && $sp_status !== 'R') {
                                ...
                                if (!empty($replacedAgentName)) {
                                    ...
                                    if ($repAgent) {
                                        ...
                                        if (strpos($sp_status, 'Suppl|') === 0) {
                                            $replacedAgentBase = isset($salary_config_raw['AS']) ? (int)$salary_config_raw['AS'] : 75000;
                                            $shift_label .= ' (Agent Supprimé/Inconnu - Base: ' . $replacedAgentBase . ')';
                                        }
                                    }
                                } else {
                                    if (strpos($sp_status, 'Suppl|') === 0) {
                                        $replacedAgentBase = isset($salary_config_raw['AS']) ? (int)$salary_config_raw['AS'] : 75000;
                                        $shift_label .= ' (Non Renseigné - Base: ' . $replacedAgentBase . ')';
                                    }
                                }
                    ...
                    if ($scObj) {
                        ...
                        if ($is_both_entrant_and_exit) {
                            ...
                            $deductions = 0;
                        } elseif ($is_special || ($is_special_salary_flag && $entrant_count > 0)) {
                            // Temps partiel ou Salaire particulier entrant : prorata sur les jours travaillés
                            $active_days = $real_active;
        ...
        foreach ($salaries as $sal) {
            ...
            if ($matched_key === null) {
                ...
                $merged_map[$matched_key] = [
                    ...
                    // Plafonnement des absences de chaque site à ses propres jours actifs
                    // pour éviter le débordement (ex: agent 24H avec ABANDON sur site muté)
                    'days_worked' => max(0, $merged_active_days - 
                        (min($existing['absences'] ?? 0, $existing['active_days'] ?? 0) + min($sal['absences'] ?? 0, $sal['active_days'] ?? 0)) - 
