...
export default function PayrollView({ setView }) {
  ...
  const calculateAgentTaxes = (s) => {
    ...
    const cnpsSalarial = payrollSettings.enable_cnps_salarial !== false ? Math.round(brut * ((payrollSettings.cnps_salarial ?? 0) / 100)) : 0;
    const cmuEmploye = payrollSettings.enable_cmu_employe !== false ? (payrollSettings.cmu_amount ?? 0) : 0;
    
    // Patronales
    const cnpsPatronal = payrollSettings.enable_cnps_patronal !== false ? Math.round(brut * ((payrollSettings.cnps_patronal ?? 0) / 100)) : 0;
    const cmuEmployeur = payrollSettings.enable_cmu_employeur !== false ? (payrollSettings.cmu_amount ?? 0) : 0;
    const accidentsTravail = payrollSettings.enable_accidents_travail !== false ? Math.round(brut * ((payrollSettings.accidents_travail ?? 0) / 100)) : 0;
    const taxeFormation = payrollSettings.enable_fdfp !== false ? Math.round(brut * ((payrollSettings.taxe_formation ?? 0) / 100)) : 0;
    const taxeApprentissage = payrollSettings.enable_taxe_apprentissage !== false ? Math.round(brut * ((payrollSettings.taxe_apprentissage ?? 0) / 100)) : 0;
    ...
    if (payrollSettings.enable_its !== false) {
      if (payrollSettings.tax_mode === 'reel_ci') {
        ...
        impotsTaxes = Math.round((brut - cnpsSalarial) * ((payrollSettings.its ?? 0) / 100));
      }
    }
  ...
  // Composant visuel SmartSearchBar
