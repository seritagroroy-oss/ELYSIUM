...
export default function Salaries() {
  ...
  const [agentActionModal, setAgentActionModal] = useState(null);
  ...
  const handleSavePaymentMethod = async (id, updatedProfile) => {
    try {
      ...
      if (res.success) {
        ...
        setPrevSalaries(prevSalaries.map(s => {
          if (s.id === id) {
            return { ...s, profile_data: JSON.stringify(updatedProfile) };
          }
          return s;
  ...
  const handleSaveImportedPayments = async (payload) => {
    try {
      const promises = payload.map(item => 
        apiCall('update_agent_profile', { agent_id: item.agent_id, profile_data: item.profile_data })
      );
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [salRes, prevSalRes] = await Promise.all([
  ...
  useEffect(() => {
    if (activeTab === 'journal') loadJournalArchives();
  }, [activeTab]);
  ...
  const calculatePayslip = useCallback((agent) => {
    ...
    const gainsCostume = (agent.cost_bonus || 0);
    ...
    const cnpsSalarial = isActive('enable_cnps_salarial') ? Math.round(salaireBrut * ((payrollSettings.cnps_salarial || 6.3) / 100)) : 0;
    const cmuEmploye = isActive('enable_cmu_employe') ? parseInt(payrollSettings.cmu_amount || 500) : 0;
    
    // Patronales
    const cnpsPatronal = isActive('enable_cnps_patronal') ? Math.round(salaireBrut * ((payrollSettings.cnps_patronal || 7.7) / 100)) : 0;
    const accidentsTravail = isActive('enable_accidents_travail') ? Math.round(salaireBrut * ((payrollSettings.accidents_travail || 2.0) / 100)) : 0;
    const taxeFormation = isActive('enable_fdfp') ? Math.round(salaireBrut * ((payrollSettings.taxe_formation || 0.6) / 100)) : 0;
    const taxeApprentissage = isActive('enable_taxe_apprentissage') ? Math.round(salaireBrut * ((payrollSettings.taxe_apprentissage || 0.4) / 100)) : 0;
    ...
    const montantPonctions = reclamations
      .filter(r => r.mois_concerne === period && r.agent_nom === agent.name && ['Clôturé', 'Transmis'].includes(r.statut) && (r.type_erreur || r.motif || r.reclamation_categorie || '').toLowerCase() === 'ponction')
    ...
    const montantReclamations = reclamations
      .filter(r => r.mois_concerne === period && r.agent_nom === agent.name && r.statut === 'Clôturé' && motifsRecl.includes((r.type_erreur || r.motif || r.reclamation_categorie || '').toLowerCase()))
      .reduce((acc, r) => acc + (parseFloat(r.montant || r.montant_estime) || 0), 0);
  ...
  const activeSalariesList = (activeTab === 'journal' && journalMode === 'archives')
  ...
  const updatePaymentStatus = async (agentId, newStatus) => {
    try {
      const res = await apiCall('update_payment_status', { period, agent_id: agentId, status: newStatus }, 'POST');
      if (res.success) {
        setSalaries(prev => prev.map(s => s.id === agentId ? { ...s, payment_status: newStatus } : s));
      } else {
    ...
    <div style={{ paddingBottom: '60px' }}>
      ...
      {toastMessage && (
        <div style={{
          ...
          {toastMessage.type === 'success' ? <CheckCircle2 size={26} /> : <AlertTriangle size={26} />}
        ...
        <div style={{ marginTop: '24px' }}>
          ...
          {activeTab === 'dashboard' && (
            <div style={{ animation: 'slideUp 0.3s ease-out' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
                {[
                  { label: 'Masse Salariale Nette', value: salariesLoading ? 'Calcul...' : fmt(dashAgg.totalNet) + ' XOF', icon: <DollarSign size={20} />, color: '#38bdf8' },
                  { label: 'Coût Total Employeur', value: salariesLoading ? 'Calcul...' : fmt(dashAgg.totalCout) + ' XOF', icon: <Building size={20} />, color: '#f97316' },
                  { label: 'Agents Payés', value: salariesLoading ? 'Calcul...' : dashAgg.count, icon: <Users size={20} />, color: '#34d399' },
                  { label: 'Personnel admin. payé', value: salariesLoading ? 'Calcul...' : dashAgg.adminCount + ' agent(s)', icon: <Users size={20} />, color: '#a78bfa' },
                ].map((kpi) => (
            ...
            <div className="glass-panel" style={{ animation: 'slideUp 0.3s ease-out' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', borderBottom: '1px solid var(--border)', paddingBottom: '12px' }}>
                <h3 style={{ margin: 0, color: '#34d399', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <CheckCircle2 size={24} /> {recTab === 'actuel' ? `Réclamations Clôturées / Validées (${fmtPeriod(latestMonth)})` : 'Archives des Réclamations Validées'}
                        ...
                        return archiveMonths.map(m => (
                          <tr key={m}>
                            ...
                            <td style={{ textAlign: 'right' }}>
                              <button
                                onClick={() => setArchivedRecMonthView(m)}
              ...
              {/* MODAL ARCHIVE VIEW */}
              {archivedRecMonthView && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15,23,42,0.85)', backdropFilter: 'blur(8px)', zIndex: 100000, display: 'flex', flexDirection: 'column', padding: '40px' }}>
                  <div style={{ maxWidth: '1200px', width: '100%', margin: '0 auto', background: '#1e293b', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.1)', overflow: 'hidden', display: 'flex', flexDirection: 'column', maxHeight: '100%', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)' }}>
                    <div style={{ padding: '24px', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.2)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                        <div style={{ background: 'linear-gradient(135deg, #a855f7, #7e22ce)', padding: '12px', borderRadius: '12px', display: 'flex' }}>
                          <Archive size={24} color="white" />
                        </div>
                        <div>
                          <h2 style={{ margin: 0, color: 'white', fontSize: '1.4rem' }}>Archives de Réclamations</h2>
                          <div style={{ color: 'var(--muted)', textTransform: 'capitalize', fontSize: '0.95rem', marginTop: '4px' }}>Mois de {fmtPeriod(archivedRecMonthView)}</div>
                        </div>
                      </div>
                      <button onClick={() => setArchivedRecMonthView(null)} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', color: 'white', width: '40px', height: '40px', borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}>
                        <X size={20} />
                      </button>
                    </div>
                    <div style={{ padding: '24px', overflowY: 'auto', flex: 1 }}>
                      <div className="table-container">
                        <table className="custom-table">
                          ...
                          <tbody>
                            {(() => {
                              let recsValidees = allValidees.filter(r => r.mois_concerne === archivedRecMonthView).sort((a, b) => b.mois_concerne.localeCompare(a.mois_concerne));
                              if (reclamationSearch) {
                                const q = reclamationSearch.toLowerCase();
                                recsValidees = recsValidees.filter(r => 
                                  (r.agent_nom && r.agent_nom.toLowerCase().includes(q)) || 
                              ...
                              if (recsValidees.length === 0) return <tr><td colSpan={7} style={{ textAlign: 'center', padding: '30px' }}>Aucune donnée correspondante</td></tr>;
                              return recsValidees.map(r => {
                                const isAbsence = r.type_erreur === 'Absence' && r.action_demandee === 'A payer';
                                return (
                                  <tr key={r.id}>
                                    ...
                                    <td>
                                      {isAbsence ? (
                                        <span style={{ background: 'rgba(52,211,153,0.2)', color: '#34d399', padding: '4px 8px', borderRadius: '4px', fontSize: '0.8rem', fontWeight: 'bold' }}>Absences justifiées</span>
                                      ) : (
                                        <span style={{ color: 'var(--muted)', fontSize: '0.8rem' }}>{r.action_demandee} (Manuel)</span>
                                      )}
                                    </td>
            ...
            <div className="glass-panel" style={{ animation: 'slideUp 0.3s ease-out' }}>
                      ...
                      <>
                        ...
                        {journalActuelLoading ? (
                          ...
                          <div className="table-container" style={{ maxHeight: '600px', overflowY: 'auto' }}>
                            ...
                            <th style={{ textAlign: 'right', width: '1%', whiteSpace: 'nowrap', padding: '8px 16px' }} title="CNPS Salarial">CNPS</th>
                            <th style={{ textAlign: 'right', width: '1%', whiteSpace: 'nowrap', padding: '8px 16px' }} title="CMU Salarial">CMU</th>
                            <th style={{ textAlign: 'right', width: '1%', whiteSpace: 'nowrap', padding: '8px 16px' }}>ITS</th>
                            <th style={{ textAlign: 'right', width: '1%', whiteSpace: 'nowrap', padding: '8px 16px' }} title="Accident du Travail">AT</th>
                            <th style={{ textAlign: 'right', width: '1%', whiteSpace: 'nowrap', padding: '8px 16px' }}>FDFP</th>
                            <th style={{ textAlign: 'right', width: '1%', whiteSpace: 'nowrap', padding: '8px 16px' }} title="Apprentissage">Appr.</th>
                            ...
                            {journalActuelData.filter(s =>
                              ...
                              if (pm === 'Mobile Money') PaymentIcon = <Smartphone size={14} color="#f97316" title="Mobile Money" style={{ marginLeft: '6px' }} />;
                              else if (pm === 'Virement Bancaire') PaymentIcon = <Building size={14} color="#3b82f6" title="Virement Bancaire" style={{ marginLeft: '6px' }} />;
                              return (<tr key={s.id}>
                                ...
                                <td style={{ textAlign: 'right', padding: '6px 16px' }}>{fmt(p.cnpsSalarial)}</td>
                                <td style={{ textAlign: 'right', padding: '6px 16px' }}>{fmt(p.cmuEmploye)}</td>
                                <td style={{ textAlign: 'right', padding: '6px 16px' }}>{fmt(p.impotsTaxes)}</td>
                                <td style={{ textAlign: 'right', padding: '6px 16px' }}>{fmt(p.accidentsTravail)}</td>
                                <td style={{ textAlign: 'right', padding: '6px 16px' }}>{fmt(p.taxeFormation)}</td>
                                <td style={{ textAlign: 'right', padding: '6px 16px' }}>{fmt(p.taxeApprentissage)}</td>
                                ...
                                <td style={{ textAlign: 'center', color: 'var(--muted)', padding: '6px 16px' }}>-</td>
                              </tr>);
                            })}
                          </tbody></table>
                          </div>
              ...
              {journalMode === 'archives' && (
                ...
                <div className="table-container" style={{ maxHeight: '600px', overflowY: 'auto' }}>
                  ...
                  <th style={{ textAlign: 'right', width: '1%', whiteSpace: 'nowrap', color: '#10b981', padding: '8px 16px' }}>Brut</th>
                  <th style={{ textAlign: 'right', width: '1%', whiteSpace: 'nowrap', padding: '8px 16px' }}>CNPS</th>
                  <th style={{ textAlign: 'right', width: '1%', whiteSpace: 'nowrap', padding: '8px 16px' }}>CMU</th>
                  <th style={{ textAlign: 'right', width: '1%', whiteSpace: 'nowrap', padding: '8px 16px' }}>ITS</th>
                  <th style={{ textAlign: 'right', width: '1%', whiteSpace: 'nowrap', padding: '8px 16px' }}>AT</th>
                  <th style={{ textAlign: 'right', width: '1%', whiteSpace: 'nowrap', padding: '8px 16px' }}>FDFP</th>
                  <th style={{ textAlign: 'right', width: '1%', whiteSpace: 'nowrap', padding: '8px 16px' }}>Appr.</th>
            ...
            return (
              <div className="glass-panel printable-section" style={{ animation: 'slideUp 0.3s ease-out', background: 'var(--card-bg)' }}>
                ...
                <div className="print-only" style={{ display: 'none' }}>
                  <h2 style={{ color: 'black', textAlign: 'center', marginBottom: '20px' }}>Liste des Moyens de Paiements - {period}</h2>
                </div>
                    ...
                    return (
                      <div key={group} style={{ marginBottom: '30px', display: isVisible ? 'block' : 'none' }} className={isVisible ? "print-avoid-break" : "no-print"}>
                        ...
                        <table className="custom-table print-friendly">
                          <thead><tr><th style={{ width: '30%' }}>Agent</th><th style={{ width: '25%' }}>Poste</th><th style={{ width: '20%' }}>Site</th><th style={{ width: '25%' }}>Détails de paiement</th></tr></thead>
                          <tbody>
                            {groupAgents.map(s => {
                              let prof = {};
                              try { prof = typeof s.profile_data === 'string' ? JSON.parse(s.profile_data) : (s.profile_data || {}); } catch(e){}
                                    ...
                                    <div>
                                      ...
                                      <select 
                                        ...
                                        style={{
                                          ...
                                          fontSize: '0.75rem',
                                          fontWeight: 'bold',
                                          background: s.payment_status === 'paid' ? 'rgba(34, 197, 94, 0.2)' : s.payment_status === 'failed' ? 'rgba(239, 68, 68, 0.2)' : 'rgba(234, 179, 8, 0.2)',
                                          color: s.payment_status === 'paid' ? '#22c55e' : s.payment_status === 'failed' ? '#ef4444' : '#eab308',
                                          cursor: 'pointer',
                                          marginRight: '8px'
                                        }}
                                      >
          ...
          {activeTab === 'masse_salariale' && (
            salariesLoading ? (
              ...
              <MasseSalariale salaries={salaries} period={period} payrollSettings={payrollSettings} />
            )
          )}
      ...
      {printFicheModal && (() => {
        const s = printFicheModal;
        const p = calculatePayslip(s);
        const profileData = parseProfileData(s.profile_data);
        const pm = profileData.payment_method || 'Especes';
        const phone = profileData.payment_phone ? `(${profileData.payment_phone_prefix || ''} ${profileData.payment_phone})` : '';
        const paymentInfo = pm === 'Mobile Money' ? `MONEY - ${profileData.payment_operator || 'Inconnu'} ${phone}` : (pm === 'Virement Bancaire' ? `BANQUE - ${profileData.bank_name || 'Inconnu'} (${profileData.bank_account || ''})` : pm);

        // Récupérer le motif des réclamations
        const agentRecl = reclamations.filter(r => r.mois_concerne === period && r.agent_nom === s.name && r.statut === 'Clôturé');

        return (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000000, padding: '20px' }}>
            <div className="printable-section" style={{ width: '100%', maxWidth: '800px', background: 'white', color: 'black', padding: '40px', borderRadius: '8px', maxHeight: '90vh', overflowY: 'auto' }}>
              <div className="no-print" style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginBottom: '20px' }}>
                <button className="btn" onClick={() => setPrintFicheModal(null)} style={{ background: '#f1f5f9', color: '#334155', border: '1px solid #cbd5e1' }}>Fermer</button>
              ...
              <div style={{ textAlign: 'center', marginBottom: '30px', borderBottom: '2px solid #000', paddingBottom: '10px' }}>
                <h1 style={{ margin: '0 0 10px 0', fontSize: '1.5rem', textTransform: 'uppercase' }}>Fiche de Pointage et de Paie</h1>
                <h2 style={{ margin: '0 0 5px 0', fontSize: '1.2rem', fontWeight: 'normal' }}>Agent : <strong>{s.name}</strong></h2>
                <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: 'normal' }}>Période : Du {payrollSettings.cycle_start} du mois précédent au {payrollSettings.cycle_end} du mois en cours</h3>
              </div>

              <table className="print-friendly" style={{ width: '100%', marginBottom: '20px', borderCollapse: 'collapse' }}>
                <tbody>
                  <tr><th style={{ width: '60%', padding: '8px', border: '1px solid #ccc', background: '#f8fafc' }}>Salaire de Base</th><td style={{ padding: '8px', border: '1px solid #ccc', textAlign: 'right', fontWeight: 'bold' }}>{fmt(p.salaireBase)} FCFA</td></tr>
                  <tr><th style={{ padding: '8px', border: '1px solid #ccc', background: '#f8fafc' }}>Heures Supplémentaires</th><td style={{ padding: '8px', border: '1px solid #ccc', textAlign: 'right' }}>{fmt(p.gainsHS)} FCFA</td></tr>
                  <tr><th style={{ padding: '8px', border: '1px solid #ccc', background: '#f8fafc' }}>Primes (Site, Ancienneté, Variables)</th><td style={{ padding: '8px', border: '1px solid #ccc', textAlign: 'right' }}>{fmt(p.primeSite + p.primeAnciennete + p.primeVariable + p.gainsCostume)} FCFA</td></tr>
                  <tr><th style={{ padding: '8px', border: '1px solid #ccc', background: '#f8fafc' }}>Retenues Absences / Abandons / MAP</th><td style={{ padding: '8px', border: '1px solid #ccc', textAlign: 'right', color: '#ef4444' }}>-{fmt(p.retenuesAbsences + p.retenuesSanctions)} FCFA</td></tr>
                  <tr><th style={{ padding: '8px', border: '1px solid #ccc', background: '#f8fafc' }}>Ponctions Manuelles</th><td style={{ padding: '8px', border: '1px solid #ccc', textAlign: 'right', color: '#f97316' }}>-{fmt(p.montantPonctions)} FCFA</td></tr>
                  <tr><th style={{ padding: '8px', border: '1px solid #ccc', background: '#f8fafc' }}>Taxes et Impôts (Salarial)</th><td style={{ padding: '8px', border: '1px solid #ccc', textAlign: 'right', color: '#ef4444' }}>-{fmt(p.totalRetenuesFiscales)} FCFA</td></tr>
                  <tr><th style={{ padding: '8px', border: '1px solid #ccc', background: '#f8fafc' }}>Remboursements Prêts / Avances</th><td style={{ padding: '8px', border: '1px solid #ccc', textAlign: 'right', color: '#ef4444' }}>-{fmt(p.totalDeductionsNettes)} FCFA</td></tr>
                  <tr><th style={{ padding: '8px', border: '1px solid #ccc', background: '#f8fafc' }}>Réclamations Validées</th><td style={{ padding: '8px', border: '1px solid #ccc', textAlign: 'right', color: '#22c55e' }}>+{fmt(p.montantReclamations)} FCFA</td></tr>
                  <tr><th style={{ padding: '12px 8px', border: '1px solid #000', background: '#e2e8f0', fontSize: '1.2rem' }}>NET À PAYER</th><td style={{ padding: '12px 8px', border: '1px solid #000', textAlign: 'right', fontSize: '1.2rem', fontWeight: 'bold' }}>{fmt(p.netAPayer)} FCFA</td></tr>
                </tbody>
              </table>

              <div style={{ marginTop: '20px' }}>
                <h3 style={{ borderBottom: '1px solid #ccc', paddingBottom: '5px' }}>Détail des Réclamations</h3>
                {agentRecl.length > 0 ? (
                  <ul style={{ paddingLeft: '20px' }}>
                    {agentRecl.map((r, i) => <li key={i}>{r.type_erreur || r.motif} : {fmt(r.montant || r.montant_estime)} FCFA</li>)}
                  </ul>
                ) : (
                  <p style={{ fontStyle: 'italic', color: '#666' }}>Aucune réclamation validée ce mois.</p>
                )}
              </div>

              <div style={{ marginTop: '30px', padding: '15px', border: '2px dashed #ccc', background: '#f8fafc' }}>
                <h3 style={{ margin: '0 0 10px 0', fontSize: '1.1rem' }}>Informations de Paiement</h3>
                <p style={{ margin: 0, fontSize: '1.2rem', fontWeight: 'bold', color: '#0f172a' }}>{paymentInfo}</p>
              </div>
      ...
      {viewDatesModal && (() => {
        ...
        const recDates = custom.dates || viewDatesModal.dates || '';
        const datesArray = recDates ? recDates.split(',').map(d => d.trim()).filter(Boolean) : [];
