...
import { Trash, Edit, Check, X, AlertTriangle, ArrowLeftRight, Clock, HelpCircle, Save, Loader2, ChevronLeft, ChevronRight, Star, TrendingUp, Shield, ShieldAlert, Users, ChevronDown, Printer, RotateCcw, Briefcase, Search, Settings, Edit2, ExternalLink, CalendarDays, Lock, Unlock } from 'lucide-react';
...
export default function DashboardTable({
  ...
  const handleDragEnd = async (event) => {
    ...
    const destSubsiteId = over.id;
    const sourceSubsiteId = active.data.current?.subsiteId;

    if (destSubsiteId !== sourceSubsiteId) {
      try {
        const response = await apiCall('move_agent_zone', {
          agent_id: agentId,
          new_subsite_id: destSubsiteId
        });
        ...
        if (response.success) {
          ...
          alert(response.error || "Une erreur est survenue lors du déplacement.");
        }
      } catch (err) {
      ...
      {(() => {
        ...
        return sortedSiteData.map(subsite => {
          ...
          const renderAgentRows = (agent, agentIdx) => {
            ...
            // 1. Forcer l'affichage de toute ligne ayant déjà des pointages significatifs (on ignore 'R' pour cacher les ancres fantômes)
            if (agent.attendance) {
              ['J', 'N', 'S', 'SJ', 'SN'].forEach(code => {
                if (agent.attendance.some(a => a.shift_code === code && a.status && a.status.trim() !== '' && a.status !== 'R')) {
            ...
            let totalExitDays = 0; // Jours d'inactivité liés au statut Sortant/Abandon/Démission...
            ...
            datesList.forEach(d => {
              ...
              const isSpecialRestDay = isSpecial && !specialDays.includes(String(jsDay)) && !specialDays.includes(jsDay);
              ...
              ['J', 'N'].forEach(sc => {
                ...
                if (st === 'ENTRANT' || st === 'NON_PRESENT') hasEntrant = true;
                if (st.startsWith('M|') || st.startsWith('PM|')) hasMutation = true;
              ...
              if (!isCpDay) {
                if (hasAbandon) { totalA++; totalExitDays++; }
                if (hasEntrant) {
                  // Pour les agents à Temps Partiel, on ne soustrait des jours ENTRANT
                  // que les jours qui étaient effectivement des jours de travail programmés
                  if (!isSpecial || !isSpecialRestDay) {
                    totalEntrant++;
                  }
                }
                if (hasMutation) {
                  if (!isSpecial || !isSpecialRestDay) {
                    totalEntrant++; // Proratise totalP de la même façon
                  }
                }
            ...
            const specialBase = (() => {
              if (agent.profile_data && agent.profile_data.special_service) {
                ...
                return datesList.filter(d => {
                  const jsDay = d.getDay() || 7;
                  return specialDays.includes(String(jsDay)) || specialDays.includes(jsDay);
                }).length;
              }
              return 30;
            })();
            ...
            // Ajustement calendaire mois de 31 jours pour agents sortants :
            // Le jour en trop du cycle est absorbé par la période d'inactivité post-départ.
            if (datesList.length > 30 && totalExitDays > 0) {
              const surplus = datesList.length - 30;
              const exitAdjust = Math.min(totalExitDays, surplus);
              totalA = Math.max(0, totalA - exitAdjust);
            }

            // Ajustement calendaire mois de 31 jours pour agents entrants :
            // Le jour en trop du cycle est absorbé par la période d'inactivité pré-arrivée (ENTRANT).
            if (datesList.length > 30 && totalEntrant > 0) {
              const surplus = datesList.length - 30;
              const entrantAdjust = Math.min(totalEntrant, surplus);
              totalEntrant = Math.max(0, totalEntrant - entrantAdjust);
            }

            let monthAdj = specialBase - datesList.length;
            if (datesList.length !== specialBase) {
                let rowIsMutated = false;
                datesList.forEach(d => {
                   const dk = formatDateKey(d);
                   ['J', 'N'].forEach(sc => {
                      const st = String(attMap[sc]?.[dk] ?? '');
                      if (st.startsWith('M|') || st.startsWith('PM|')) rowIsMutated = true;
                   });
                });
                if (rowIsMutated) {
                    let workedDays = datesList.length - totalEntrant - totalExitDays;
                    monthAdj = monthAdj * (workedDays / datesList.length);
                }
            }
            ...
            let totalP = Math.max(0, datesList.length - totalA - totalMAP - totalEntrant - totalPermission) + Math.round(monthAdj) + totalSpecialExtra;
            if (agent.is_mutated && agent.days_consumed_by_origin !== undefined) {
                // Règle du site d'origine prioritaire : la destination ne prend que ce qu'il reste pour atteindre 30
                const baseAllowed = Math.max(0, 30 - agent.days_consumed_by_origin);
                totalP = Math.max(0, baseAllowed - totalA - totalMAP - totalPermission) + totalSpecialExtra;
            }
            const rows = shiftRows.map((sc, scIdx) => {
              ...
              return (
                <DraggableAgentRow key={`${agent.id || agent.name || agent.nom}-${sc}`} agent={agent} subsiteId={subsite.id} scIdx={scIdx}>
                  {({ setNodeRef, listeners, attributes, style: dragStyle }) => (
                    <tr ref={setNodeRef} id={scIdx === 0 ? `agent-row-${agent.id || agent.name || agent.nom}` : undefined} style={{
                      background: trBg,
                      '--c': trColorVar,
                      ...dragStyle
                    }}>
                                    ...
                                    {(!agent.is_mutated && !agent.is_extra && !agent.is_releve) && (
                                      <>
                                        ...
                                        {agent.profile_data?.special_service && (
                                          <button
                                            onClick={requireEditMode((e) => { e.stopPropagation(); onEditSpecialService && onEditSpecialService(agent); })}
                                            className="btn"
                                            style={{ 
                                              padding: '3px 6px', 
                                              borderRadius: '8px', 
                                            ...
                                            title="Modifier la configuration Temps Partiel"
                                          >
                                            <Edit2 size={12} />
                                          </button>
                                        )}
                  ...
                  {scIdx === 0 ? (
                    <td rowSpan={shiftRows.length} className={`agent-rowspan-cell ${bottomCellClass}`} style={{ verticalAlign: 'middle', padding: '0 4px', width: '65px', minWidth: '65px', maxWidth: '65px', position: 'sticky', left: '250px', background: agent.is_extra ? (localStorage.getItem('pontage_extra_name_bg') || '#2a121a') : (agent.is_releve ? (localStorage.getItem('pontage_releve_name_bg') || '#2a121a') : '#0b1220'), zIndex: 40, borderRight: '1px solid var(--border)' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', padding: '4px 0' }}>
                        ...
                        {!agent.is_mutated && !isArchiveMode && !isVerificationMode && !agent.is_releve && (
                          <button
                  ...
                  {(() => {
                    ...
                    for (let dIdx = 0; dIdx < datesList.length; dIdx++) {
                      ...
                      const isSortant = isMainRow && (
                        ['ABANDON', 'DEMISSION', 'RETIRE', 'LICENCIE', 'LICENCIE_ADMIN', 'FIN_CONTRAT'].includes(status) || status.startsWith('SORTANT_') ||
                        ['ABANDON', 'DEMISSION', 'RETIRE', 'LICENCIE', 'LICENCIE_ADMIN', 'FIN_CONTRAT'].includes(attMap['J']?.[dk]) || String(attMap['J']?.[dk]).startsWith('SORTANT_') ||
                        ['ABANDON', 'DEMISSION', 'RETIRE', 'LICENCIE', 'LICENCIE_ADMIN', 'FIN_CONTRAT'].includes(attMap['N']?.[dk]) || String(attMap['N']?.[dk]).startsWith('SORTANT_')
                      );
                      ...
                      // Fusion verticale pour ENTRANT, CP, et SORTANT : on ne rend la cellule que sur la première ligne principale (scIdx === 0)
                      if ((isEntrant || isCp || isSortant) && scIdx > 0 && isMainRow) {
                        continue;
                      }
                      ...
                      if (isSortant) {
                        ...
                        for (let j = dIdx; j < datesList.length; j++) {
                          const tDk = formatDateKey(datesList[j]);
                          const tStatusJ = String(attMap['J']?.[tDk] ?? '');
                          const tStatusN = String(attMap['N']?.[tDk] ?? '');
                          
                          const isSortantJ = ['ABANDON', 'DEMISSION', 'RETIRE', 'LICENCIE', 'LICENCIE_ADMIN', 'FIN_CONTRAT'].includes(tStatusJ) || tStatusJ.startsWith('SORTANT_');
                          const isSortantN = ['ABANDON', 'DEMISSION', 'RETIRE', 'LICENCIE', 'LICENCIE_ADMIN', 'FIN_CONTRAT'].includes(tStatusN) || tStatusN.startsWith('SORTANT_');
                          
                          if (isSortantJ || isSortantN) count++;
                          else break;
                        }
                        sortantSpanCount = count;
                      } else {
                        sortantRendered = false;
                      }
                      ...
                      if (agent.status_change && !isMutated && !isPrevMutated && !isSortant && !isEntrant && !isCp && status !== 'P' && !['A', 'MAP', 'M', 'AT', 'CP', 'R'].includes(status)) {
                        try {
                          const scObj = JSON.parse(agent.status_change);
                      ...
                      if (isModernTheme && !isMutated && !isPrevMutated && !isNonPresent) {
                        if (enableAnimations) {
                          ...
                          } else if (['M', 'CP', 'AT', 'MAP', 'P'].includes(status) || isCp) {
                      ...
                      cells.push(
                        <td
                          ...
                          colSpan={colSpan}
                          rowSpan={(isEntrant || isCp || isSortant) && scIdx === 0 ? shiftRows.filter(r => !r.startsWith('S')).length : 1}
                          onMouseDown={(e) => {
                            if (paintModeActive) {
            ...
            return (
              <div key={subsite.id} style={{ marginTop: '16px' }}>
                <div className="glass-panel subsite-card" style={{ padding: '10px 14px', marginBottom: '10px', display: 'flex', justifyContent: 'center', alignItems: 'center', position: 'relative', background: currentSiteMode === 'individual' ? '#ffffff' : undefined, color: currentSiteMode === 'individual' ? '#0f172a' : undefined }}>
                  ...
                  {!isMutatedGroup && !isArchiveMode && !isVerificationMode && subsite.id !== 'virtual_releves' && (
                    <div style={{ display: 'flex', gap: '8px', position: 'absolute', right: '14px' }}>
                      <button
                        ...
                        style={{ padding: '4px', background: 'transparent', border: 'none', color: '#10b981', marginRight: '8px', opacity: lockedZones.includes(subsite.id) ? 0.3 : 1 }}
                        disabled={lockedZones.includes(subsite.id)}
                        title="Effacer les marqueurs 'traité' de cette zone"
                      >
                        🧹
                      </button>
                      <button
                        onClick={() => toggleZoneLock(subsite.id)}
                        className="btn btn-secondary"
                        style={{ padding: '4px', background: 'transparent', border: 'none', color: lockedZones.includes(subsite.id) ? '#ef4444' : 'rgba(255,255,255,0.4)', marginRight: '28px' }}
                        title={lockedZones.includes(subsite.id) ? "Déverrouiller cette zone" : "Verrouiller cette zone (Bloquer l'édition)"}
                      >
                        {lockedZones.includes(subsite.id) ? <Lock size={16} /> : <Unlock size={16} />}
                      </button>
                      <button 
                        onClick={lockedZones.includes(subsite.id) ? null : requireEditMode(() => setExternalSuppModal && setExternalSuppModal({ subsite }))}
                        className="btn btn-secondary" 
                        style={{ padding: '4px', background: 'transparent', border: 'none', color: '#8b5cf6', opacity: lockedZones.includes(subsite.id) ? 0.3 : 1 }}
                        disabled={lockedZones.includes(subsite.id)}
                      ...
                      <button 
                        onClick={lockedZones.includes(subsite.id) ? null : requireEditMode(() => setMoveZoneAgent({ subsite_id: subsite.id, from_header: true }))}
                        className="btn btn-secondary" 
                        style={{ padding: '4px', background: 'transparent', border: 'none', color: '#0ea5e9', opacity: lockedZones.includes(subsite.id) ? 0.3 : 1 }}
                        disabled={lockedZones.includes(subsite.id)}
