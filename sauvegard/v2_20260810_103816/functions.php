                ...
                foreach ($agents_rows as $agent) {
                    ...
                    foreach ($dates as $date) {
                        ...
                        if ($is_24h) {
                            ...
                            if (!$is_abs_j && $sJ === 'ENTRANT') {
                                $entrant_sortant_count++;
                                $entrant_count++;
                            }
                            if (!$is_abs_n && $sN === 'ENTRANT') {
                                $entrant_sortant_count++;
                                $entrant_count++;
                            }
                    ...
                    if (count($dates) > 30) {
                        ...
                        foreach ($dates as $d_check) {
                            $sJ_c = $att_map['J'][$d_check] ?? '';
                            $sN_c = $att_map['N'][$d_check] ?? '';
                            if ($sJ_c === 'ENTRANT' || $sN_c === 'ENTRANT') $entrant_days_count++;
                        }
                    ...
                    foreach ($dates as $date) {
                        ...
                        if ($is_scheduled_day) {
                            $is_entrant_or_np = ($sJ === 'ENTRANT' || $sN === 'ENTRANT' || $sJ === 'NON_PRESENT' || $sN === 'NON_PRESENT');
                            if (!$is_entrant_or_np) {
                                $assigned_days++;
        ...
        foreach ($salaries as $sal) {
            ...
            if ($matched_key === null) {
                ...
                $merged_map[$matched_key] = [
                    ...
                    'days_worked' => min($merged_active_days, ($existing['days_worked'] ?? 0) + ($sal['days_worked'] ?? 0)),
