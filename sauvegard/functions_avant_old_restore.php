            ...
            foreach ($subsites_rows as $sub) {
                ...
                foreach ($agents_rows as $agent) {
                    ...
                    if ($scObj) {
                        ...
                        if ($is_special) {
                            $divToUse = 30; // Toujours 30 pour la paie Temps Partiel afin d'avoir le bon taux
                            $deductions = (int) round(($financial_absence_units + $map_count + $permission_count) * ($base_used_for_deductions / $divToUse));
                        } else {
        ...
        foreach ($salaries as $sal) {
            ...
            foreach ($merged_map as $key => $existing) {
                ...
                if ($salIsSpecial || $existingIsSpecial) {
                    // Les agents à Temps Partiel (même nom, même zone d'identité) 
                    // travaillant sur plusieurs sites sont automatiquement fusionnés.
                    $matched_key = $key;
                    break;
                }
            ...
            if ($matched_key === null) {
                ...
                // Pour les agents Temps Partiel, les jours supplementaires ne doivent pas
                // être comptés dans les active_days (eviter le double comptage salaire+supplement)
                $isSpecialMerge = !empty($oldest['profile_data']['special_service']) || !empty($newest['profile_data']['special_service']);
                $tp_sp_count = $isSpecialMerge ? (($oldest['sp_count'] ?? 0) + ($newest['sp_count'] ?? 0)) : 0;
                $merged_active_days = min($divisor_merge, $total_active_days - $tp_sp_count);
                ...
                // Pour les agents Temps Partiel fusionnés depuis plusieurs sites,
                // remplir multi_site_deployments pour l'affichage des mouvements dans le frontend
                $isSpecialFusion = !empty($oldest['profile_data']['special_service']) || !empty($newest['profile_data']['special_service']);
                if ($isSpecialFusion) {
                    // Construire la liste des déploiements multi-sites à partir des entrées existantes
                    $existing_deployments = $oldest['profile_data']['multi_site_deployments'] 
                        ?? [['site' => $oldest['site'], 'worked_days' => max(0, ($oldest['active_days'] ?? 0) - ($oldest['sp_count'] ?? 0))]];
                    $new_deployments = $newest['profile_data']['multi_site_deployments'] 
                        ?? [['site' => $newest['site'], 'worked_days' => max(0, ($newest['active_days'] ?? 0) - ($newest['sp_count'] ?? 0))]];
                    $merged_profile['multi_site_deployments'] = array_merge($existing_deployments, $new_deployments);
                }
