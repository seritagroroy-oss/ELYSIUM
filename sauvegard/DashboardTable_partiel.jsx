        ...
        return sortedSiteData.map(subsite => {
          ...
          const renderAgentRows = (agent, agentIdx) => {
            ...
            datesList.forEach(d => {
              ...
              if (hasAbandon || hasEntrant || hasMutation) totalRupture++;
            ...
            if (datesList.length > 30 && totalRupture > 0 && shouldRunCorrection) {
              ...
              if (totalEntrant >= adjust) {
                  ...
                  const rem = adjust - totalEntrant;
                  totalEntrant = 0;
                  totalA = Math.max(0, totalA - rem);
              }
            }
            // --- FIN CORRECTION ---
            ...
            const isStrictRuptureFor24h = totalRupture > 0 && (!isOriginMutationForAgent && !isDestinationMutation || hasAgentAbandon);
            ...
            if (is244872 && isStrictRuptureFor24h) {
              ...
              datesList.forEach(d => {
                const dk = formatDateKey(d);
                ['J', 'N'].forEach(sc => {
                  if (attMap[sc]?.[dk] === '1') totalRealWorkedUnits++;
                });
              });
              totalP = totalRealWorkedUnits + totalSpecialExtra;
            }
                ...
                <DraggableAgentRow key={`${agent.id}-${sc}`} agent={agent} subsiteId={subsite.id} scIdx={scIdx}>
                  ...
                  {(() => {
                    ...
                    for (let dIdx = 0; dIdx < datesList.length; dIdx++) {
                      ...
                      const checkSortant = (st) => ['ABANDON', 'DEMISSION', 'RETIRE', 'LICENCIE', 'LICENCIE_ADMIN', 'FIN_CONTRAT'].includes(st) || (st && String(st).startsWith('SORTANT_'));
                      const isMutated = status.startsWith('M|') || (isMainRow && (String(attMap['J']?.[dk] || '').startsWith('M|') || String(attMap['N']?.[dk] || '').startsWith('M|')));
                      const isPrevMutated = status.startsWith('PM|') || (isMainRow && (String(attMap['J']?.[dk] || '').startsWith('PM|') || String(attMap['N']?.[dk] || '').startsWith('PM|')));
                      const isSortant = checkSortant(status) || (isMainRow && (checkSortant(attMap['J']?.[dk]) || checkSortant(attMap['N']?.[dk])));
                      
                      const isEntrant = isMainRow && (['ENTRANT', 'REINTEGRATION'].includes(status) || ['ENTRANT', 'REINTEGRATION'].includes(attMap['J']?.[dk]) || ['ENTRANT', 'REINTEGRATION'].includes(attMap['N']?.[dk]));
                      let actualEntrantStatus = 'ENTRANT';
                      if (status === 'REINTEGRATION' || attMap['J']?.[dk] === 'REINTEGRATION' || attMap['N']?.[dk] === 'REINTEGRATION') {
                        actualEntrantStatus = 'RÉINTÉGRATION';
                      }
                      ...
                      // Fusion verticale pour ENTRANT, CP, SORTANT, PM et M : on ne rend la cellule que sur la première ligne principale (scIdx === 0)
                      if ((isEntrant || isCp || isSortant || isPrevMutated || isMutated) && scIdx > 0 && isMainRow) {
                        continue;
                      }
                      ...
                      if (isEntrant) {
                        ...
                        for (let j = dIdx; j < datesList.length; j++) {
                          ...
                          if (['ENTRANT', 'REINTEGRATION'].includes(tStatus)) count++;
                          else break;
                        }
                      ...
                      cells.push(
                        <td
                          ...
                          colSpan={colSpan}
                          rowSpan={(isEntrant || isCp || isSortant || isPrevMutated || isMutated) && scIdx === 0 ? shiftRows.filter(r => !r.startsWith('S')).length : 1}
                          onMouseDown={(e) => {
                            if (paintModeActive) {
