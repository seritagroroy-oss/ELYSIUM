...
switch ($action) {
    ...
    case 'save_payroll_status':
        // Persiste le statut de paie d'un agent (brouillon → valide → paye)
        if (!hasPermission('fluctuation') && !hasPermission('salaries') && !hasWritePermission('company_config')) {
            echo json_encode(['success' => false, 'message' => 'Accès refusé']);
            break;
        }
        ...
        if (!$period || !$agentName) {
            echo json_encode(['success' => false, 'message' => 'Paramètres manquants (period, agent_name)']);
            break;
        }
    ...
    case 'bulk_save_payroll_status':
        ...
        if (!$period || !is_array($updates) || empty($updates)) {
            echo json_encode(['success' => false, 'message' => 'Paramètres manquants (period, updates)']);
            break;
        }
    ...
    case 'get_payroll_statuses':
        ...
        if (!$period) {
            echo json_encode(['success' => false, 'message' => 'Paramètre period manquant']);
            break;
        }
    ...
    case 'get_salaries':
        ...
        $applyFallback = function(&$salariesData, $isArchive = false) use ($liveProfiles, $liveProfilesByName, $statuses) {
            foreach ($salariesData as &$agentData) {
                ...
                // Le statut de paiement (payé, en attente, etc.) est par contre spécifique à la période
        ...
        if (isPayrollArchived($sqlite, $companyKey, $period)) {
            ...
            $applyFallback($salaries, true); // true = isArchive car on vient de la générer pour une période clôturée
    ...
    case 'publish_period':
        ...
        // Assigner le statut de fluctuation de la période actuelle à pending_compta
    ...
    case 'apply_last_minute_correction':
        // Autoriser tout utilisateur connecté pour la flexibilité des corrections locales
        ...
        try {
            ...
            if ($action_type === 'change_mutation_destination') {
                ...
                // 2. Mettre à jour l'attendance de l'agent d'origine
                ...
                // 3. Chercher si un doublon existait déjà pour cet agent (même nom) sur un autre subsite et le supprimer
                ...
                // 4. Créer le nouvel agent dupliqué sur le bon subsite
                ...
                // 5. Générer son historique d'attendance
                ...
                foreach ($datesList as $date_str) {
                    if ($date_str < $first_mut_date) {
                        $stmtSubName = $sqlite->prepare("SELECT s.name as site_name, sub.name as zone_name FROM subsites sub LEFT JOIN sites s ON sub.site_id = s.id WHERE sub.id = ?");
                        $stmtSubName->execute([$orig_agent['subsite_id']]);
            ...
            // 6. Régénérer le snapshot de paie
            ...
            $sqlite->commit();
            echo json_encode(['success' => true, 'message' => 'Correction appliquée et État de paie actualisé !']);
        } catch (\Exception $e) {
            if (isset($sqlite) && $sqlite->inTransaction()) {
                $sqlite->rollBack();
            }
    ...
    case 'get_agent_period_details':
        ...
        try {
            $stmt = $sqlite->prepare("SELECT date, status FROM attendance WHERE agent_id = ? AND period = ? AND (status = 'A' OR status LIKE '%MAP%')");
        ...
        try {
            $stmt2 = $sqlite->prepare("SELECT date_supp, heures FROM supplementaires_externes WHERE agent_id = ? AND periode = ?");
            $stmt2->execute([$agent_id, $period]);
            while ($r = $stmt2->fetch()) {
