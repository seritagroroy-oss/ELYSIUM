                   ...
                   ");
                    ...
                    foreach ($mutated_rows as $agent) {
                        ...
                        if ($is_extra || $is_releve) {
                            ...
                            // CALCUL DES JOURS REELS CONSOMMÉS SUR LE SITE D'ORIGINE
                            // On ne compte QUE les jours réellement travaillés sur le site d'origine.
                            // Les jours M|... (jours de mutation vers le site de destination) sont exclus
                            // car ils correspondent à des jours passés AILLEURS, pas sur le site d'origine.
                            $origin_total_A = 0;
                            $origin_total_MAP = 0;
                            $origin_total_P = 0;
                            $origin_total_Entrant = 0;
                            $origin_total_Exit = 0;
                            $origin_mutation_days = 0; // Jours passés sur le site de destination
                            
                            // On regroupe par date pour éviter de compter 2x (shift J et N)
                            $origin_dates_counted = [];
                            foreach ($raw_att2 as $orig_att) {
                                $st = $orig_att['status'] ?? '';
                                $dt = $orig_att['date'] ?? '';
                                if (isset($origin_dates_counted[$dt])) continue;
                                $origin_dates_counted[$dt] = true;
                                
                                // Jour de mutation vers un autre site → compter comme jour absent de l'origine
                            ...
                            // Les jours réels de présence sur le site d'origine = jours actifs - absences - jours sur l'autre site
