...
import {
  ...
  Calendar, Briefcase, Calculator, PiggyBank, History, ArrowLeftRight
} from 'lucide-react';
...
function MutationDetailsModal({ selectedMutationDetails, onClose }) {
  ...
  const totalDays = (original.worked_days !== undefined ? original.worked_days : (original.active_days||0)) + 
                    (mutated.worked_days !== undefined ? mutated.worked_days : (mutated.active_days||0));
  ...
  const renderDaysInfo = (details, badgeColor = '#38bdf8') => {
    ...
    return (
      <div style={{ textAlign: 'right' }}>
        ...
        <div style={{ marginTop: '4px', fontSize: '0.88rem', fontWeight: '700', color: badgeColor, display: 'inline-flex', alignItems: 'center', gap: '6px', background: `${badgeColor}15`, padding: '3px 10px', borderRadius: '12px', border: `1px solid ${badgeColor}30` }}>
          <Clock size={13} /> {worked} jour(s) de service réel
        </div>
        {hasDeductions ? (
          <span style={{ fontSize: '0.74rem', color: '#64748b', display: 'block', marginTop: '6px', fontWeight: '500' }}>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Workflow des deux cartes de mutation */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ 
            ...
            <p style={{ margin: 0, fontSize: '0.92rem', color: '#94a3b8', lineHeight: '1.6' }}>
              <strong style={{ color: '#38bdf8' }}>Note comptable (Règle forfaitaire des 30 jours) :</strong> Les jours actifs sur chaque site sont ajustés de manière proratisée pour respecter le forfait mensuel universel de 30 jours {original.calendar_active_days !== undefined && mutated.calendar_active_days !== undefined ? `(${original.calendar_active_days + mutated.calendar_active_days} jours calendaires réels équivalent à ${original.active_days + mutated.active_days} jours comptables)` : ''}, garantissant une paie exacte et conforme.
            </p>
          </div>
        </div>
        ...
        <div style={{ marginTop: '32px', display: 'flex', justifyContent: 'flex-end' }}>
          <button 
            ...
            onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 8px 25px rgba(2, 132, 199, 0.6)'; }}
