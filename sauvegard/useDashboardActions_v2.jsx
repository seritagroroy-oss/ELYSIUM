  ...
  const handlePublishPeriod = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        try {
            // Le frontend ne charge plus les données massives pour les renvoyer au backend !
            // On demande simplement au backend de faire le travail de son côté.
            ...
            console.log('✅ Photo figée enregistrée avec succès (agents + pointages complets) par le serveur');
            ...
            setErrorMsg("Publication incomplète : Erreur lors de la création de la photo figée (archive_pointage). " + (err.message || err));
        }
      } else {
  ...
  const handleCreateAgentFromModal = async (agentData) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        // Recharger le tableau de bord complet pour être 100% sûr d'avoir le nouvel agent (exactement comme un F5)
        await loadDashboardData(true);
        
        // Ensuite, on ferme la modale (cela enlève le bouton de chargement)
        setShowAddAgent(false);
      } else {
        setErrorMsg(res.message);
      }
