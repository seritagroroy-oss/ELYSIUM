...
function buildSiteDataSnapshot($sqlite, $serviceKey, $period, $siteOrder = [])
{
    ...
    if (!$has_extras) {
        $sites[] = ['id' => 'site_extras', 'name' => 'ðŸŒŸ EXTRA BUREAU'];
    }
    if (!$has_releves) {
        $sites[] = ['id' => 'site_releves', 'name' => 'ðŸ”„ Vivier des relÃ¨ves'];
    }
    if (!array_filter($sites, fn($s) => $s['id'] === 'site_extras_sur_site')) {
        $sites[] = ['id' => 'site_extras_sur_site', 'name' => 'ðŸŒŸ EXTRA SUR SITE'];
    }
    if (!$has_admin)
        $sites[] = ['id' => 'site_administration', 'name' => 'ðŸ¢ Administration'];
    if (!$has_itc)
...
$settings_raw = getServiceDataSql($serviceKey, 'settings', ['cycle_start' => 21, 'cycle_end' => 20]);
        ...
        if (!$has_extras) {
            $sites_rows[] = ['id' => 'site_extras', 'name' => 'ðŸŒŸ EXTRA BUREAU'];
        }
        if (!$has_releves) {
            $sites_rows[] = ['id' => 'site_releves', 'name' => 'ðŸ”„ Vivier des relÃ¨ves'];
        }
        if (!array_filter($sites_rows, fn($s) => $s['id'] === 'site_extras_sur_site')) {
            $sites_rows[] = ['id' => 'site_extras_sur_site', 'name' => 'ðŸŒŸ EXTRA SUR SITE'];
        }
        if (!$has_admin)
            $sites_rows[] = ['id' => 'site_administration', 'name' => 'ðŸ¢ Administration'];
        if (!$has_itc)
        ...
        foreach ($sites_rows as $site) {
            ...
            // Pour EXTRA SUR SITE : toujours inclure le subsite par dÃ©faut mÃªme si des sous-sites
            // rÃ©els existent, pour ne pas manquer les agents affectÃ©s Ã  "default_site_extras_sur_site"
            if ($site['id'] === 'site_extras_sur_site') {
                $has_default = false;
            ...
            foreach ($subsites_rows as $sub) {
                ...
                foreach ($agents_rows as $agent) {
                    ...
                    if ($is244872 && $totalRuptureBackend > 0) {
                        ...
                        // Ajuster pour combler l'Ã©cart par rapport Ã  30
                        $absences = max(0, 30 - $totalRealWorkedUnits);
                        // On remet les compteurs de rupture Ã  0 pour Ã©viter la double retenue
                        $entrant_sortant_count = 0;
                        $entrant_count = 0;
                    }
                    ...
                    // Normalisation des jours de CP sur une base de 30 jours (pour gÃ©rer les mois de 31j ou 28j/29j)
                    ...
                    $sp_count = 0;
                    $sp_details = [];
                    // 1. Pointages explicitement dans la ligne SupplÃ©mentaire
                    foreach (['S', 'SJ', 'SN'] as $sp_key) {
                        foreach ($dates as $date) {
                            $sp_status = $att_map[$sp_key][$date] ?? '';
                            if ($sp_status !== '' && $sp_status !== 'A' && $sp_status !== 'R') {
                                $sp_count++;
                                $shift_label = 'SupplÃ©mentaire';
                                if ($sp_key === 'SJ')
                                    $shift_label = 'SupplÃ©mentaire Jour';
                                elseif ($sp_key === 'SN')
                                    $shift_label = 'SupplÃ©mentaire Nuit';
                    ...
                    // 2. Pointages SupplÃ©mentaires effectuÃ©s en dÃ©ploiement (sur les lignes principales J ou N)
                    foreach (['J', 'N'] as $main_key) {
                        foreach ($dates as $date) {
                            ...
                            if (strpos($main_status, 'EXT_1|') === 0 || strpos($main_status, 'REL_1|') === 0 || strpos($main_status, 'M_1|') === 0) {
                                ...
                                $shift_label = 'Suppl. DÃ©ployÃ© (' . ($main_key === 'J' ? 'Jour' : 'Nuit') . ')';
                                $sp_details[] = ['date' => $date, 'shift' => $shift_label . ' sur ' . $dest, 'replacedAgentBase' => $replacedAgentBase];
                            }
                        }
                    }

                    // 3. Jours de prÃ©sence normale effectuÃ©s par les agents en Temps Partiel hors de leur planning
                    ...
                    if ($scObj) {
                        ...
                        // Ancienne mÃ©thode : on ne dÃ©duit plus special_days_count des jours actifs

                        $prorata_base = (int) round($base_old * ($active_days_old / $divToUse)) + (int) round($base_new * ($active_days_new / $divToUse));
                        
                        // Calcul exact des dÃ©ductions par pÃ©riode (selon la date de l'absence)
                        ...
                        // Calcul exact des gains par pÃ©riode
                        $gains = 0;
                        ...
                        $base_used_for_deductions = $base; // UtilisÃ© pour les Ã©ventuels calculs annexes, mais deductions et gains sont dÃ©jÃ  fixÃ©s
                    } else {
                        if ($is_special && !$is_special_salary_flag) {
                            // Temps partiel SANS salaire particulier : prorata sur les jours travaillÃ©s
                            $active_days = $real_active;
                            ...
                            // === RÃˆGLE ABSOLUE : salaire particulier (Configuration Entreprise) ===
                            // Le salaire fixe prime TOUJOURS, qu'il soit temps partiel ou non.
                            // On n'applique AUCUN prorata.
                            $prorata_base = $base;
                            $active_days = $real_active;
                        } else {
                            // Si l'agent n'a pas de jours ENTRANT et a travaillÃ© tous ses jours prÃ©vus, sa base = base complÃ¨te
                        ...
                        // ENTRANT exclus des dÃ©ductions : l'agent n'Ã©tait pas encore en poste, ce n'est pas une absence
                        if ($is_special) {
                            $deductions = 0;
                        } else {
                    ...
                    $salaries[] = [
                        ...
                        'days_worked' => $is_special ? $real_active : max(0, $active_days - ($absences + $map_count + $permission_count)),
