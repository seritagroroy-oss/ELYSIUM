                        ...
                        foreach ($sp_details as $spd) {
                            ...
                            if (isset($spd['replacedAgentBase']) && $spd['replacedAgentBase'] !== null) {
                                ...
                                if ($replaced_base > $agent_base) {
                                    ...
                                    $gains += (int) round(($replaced_base - $agent_base) / $divisor);
                                } else {
                                    // Scénario B : Poste inférieur -> S'adapte au taux remplacé
                                    $gains += (int) round($replaced_base / $divisor);
                                }
                            } else {
                                // Scénario C : Aucun remplacement
                                $gains += (int) round($agent_base / $divisor);
                            }
                        }
        ...
        foreach ($salaries as $sal) {
            ...
            if ($matched_key === null) {
                ...
                if ($total_active_days > $divisor_merge) {
                    $ratio = $divisor_merge / $total_active_days;
                    $adjusted_old_active = (int) round($adjusted_old_active * $ratio);
...
function closeFluctuationForPeriod($companyId, $period, $closed_by = 'Auto-Archivage (PC)') {
    ...
    // 4. Charger les agents et faire les comptes
    ...
    foreach ($allAgents as $agent) {
        ...
        $agent_net  = ($base - $deductions + $gains);
        
        $sites_rentability[$site_name]['salary_expense'] += $agent_net;
        
        if (strpos(strtolower($site_name), 'administration') !== false) {
            $ms_admin += $agent_net;
            $admin_count++;
        } else {
            $ms_agents += $agent_net;
