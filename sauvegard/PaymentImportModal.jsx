...
export default function PaymentImportModal({ salaries, onClose, onSave }) {
  ...
  const generatePreview = () => {
    ...
    setTimeout(() => {
      ...
      salaries.forEach(s => {
        exactMatchMap.set(s.name.toLowerCase().trim(), s);
        let pd = {};
        try { pd = JSON.parse(s.profile_data || '{}'); } catch(e) {}
      ...
      const processChunk = () => {
        ...
        for (let index = currentIndex; index < endIndex; index++) {
          ...
          if (exactMatchMap.has(lowerName)) {
            bestMatch = exactMatchMap.get(lowerName);
            highestSim = 1.0;
            matchStatus = 'exact';
          } else {
            salaries.forEach(s => {
          ...
          results.push({
            ...
            validated: matchStatus === 'exact' && !isDuplicate && !isGlobalDuplicate && !isInvalidFormat
          });
        }
          ...
          {step === 2 && (
            <div style={{ animation: 'fadeIn 0.3s' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '16px' }}>
                ...
                <button className="btn btn-primary" onClick={generatePreview} disabled={isProcessing} style={{ padding: '12px 24px', borderRadius: '10px', display: 'flex', alignItems: 'center', gap: '8px', minWidth: '220px', justifyContent: 'center' }}>
                  {isProcessing ? (
                    <>
                      ...
                      <tr key={p.id} style={{ 
                        ...
                        <td>
                          {p.isGlobalDuplicate ? (
                            ...
                            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', color: '#ef4444', fontSize: '0.85rem', fontWeight: 'bold' }}>
                              <AlertTriangle size={14} /> Format Invalide (10 chiffres requis)
                            </span>
                          ) : p.matchStatus === 'exact' ? (
