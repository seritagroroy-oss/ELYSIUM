...
switch ($action) {
    ...
    case 'add_agent':
        ...
        $db = getScopedData($serviceKey);
        if ($period !== '') {
            applyShiftDefaultsForPeriod($db, $new_agent_id, $period, $shift_type, $admin_schedule, $special_service, $special_service_days);
            ...
            if ($debut_date && isset($db['attendance'][$period][$new_agent_id])) {
                ...
                // on remplace par NON_PRESENT sur le shift principal et on supprime l'autre shift
                ...
                foreach ($allDatesInAttendance as $attDate) {
                    if ($attDate < $debut_date) {
                        // Remplacer par NON_PRESENT sur le shift principal (pour griser individuellement sans fusion)
                        $db['attendance'][$period][$new_agent_id][$targetShiftForEntrant][$attDate] = 'NON_PRESENT';
