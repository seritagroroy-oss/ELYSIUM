    ...
    case 'apply_mutation':
        ...
        while ($cursor <= $end_time) {
            ...
            if ($cursor < $start_time) {
                ...
                if ($stype === 'Nuit') {
                    $batchRows[] = [$new_agent_id, $date_str, 'N', '1', $company_id, $serviceKey, $period];
                } elseif ($stype === 'Jour') {
                    $batchRows[] = [$new_agent_id, $date_str, 'J', '1', $company_id, $serviceKey, $period];
                } else {
                    $cycle = 1; $work = 1;
                    $stype_lower = strtolower($stype);
                    if ($stype_lower === '24h') { $cycle = 2; $work = 1; }
                    ...
                    $batchRows[] = [$new_agent_id, $date_str, 'J', $val, $company_id, $serviceKey, $period];
                    $batchRows[] = [$new_agent_id, $date_str, 'N', $val, $company_id, $serviceKey, $period];
                    $cycle_counter++;
                }
            }
    ...
    case 'delete_external_supp':
        ...
        if ($supp) {
            ...
            for ($i = 0; $i < $days; $i++) {
                ...
                $sqlite->prepare("DELETE FROM attendance WHERE agent_id = ? AND date = ? AND status LIKE 'Suppl|%'")
                       ->execute([$agent_id, $cur_date]);
            }
