...
switch ($action) {
    ...
    case 'mark_agent_entrant':
        ...
        $entrant_motif = $data['entrant_motif'] ?? 'ENTRANT';
        $period     = $data['period'] ?? date('Y-m');
    ...
    case 'apply_mutation':
        ...
        // Dupliquer les congés globaux futurs (pointage_leaves) vers le nouvel agent
        $stmtLeaves = $sqlite->prepare("SELECT * FROM pointage_leaves WHERE agent_id = ? AND end_date >= ?");
        $stmtLeaves->execute([$agent_id, $start_date]);
        $leaves_to_transfer = $stmtLeaves->fetchAll();
        foreach ($leaves_to_transfer as $lv) {
            $new_leave_id = 'lv_' . uniqid() . '_' . rand(100,999);
            $created_at = date('Y-m-d H:i:s');
            try {
                $sqlite->prepare("INSERT INTO pointage_leaves (id, agent_id, start_date, end_date, type, status, company_id, service_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")
                       ->execute([
                           $new_leave_id, $new_agent_id, $lv['start_date'], $lv['end_date'],
                           $lv['type'], $lv['status'], $lv['company_id'], $lv['service_id'], $created_at
                       ]);
            } catch (Exception $e) {}
        }
        ...
        foreach ($future_rows as $row) {
            $future_att_raw[$row['shift_code']][$row['date']] = $row['status'];
        }
        
        // Lissage (smoothing) pour combler les jours de repos (R) entre deux statuts identiques.
        // Utile lorsqu'un agent passe d'un roulement (ex: 24h) à un cycle continu (Jour).
        $future_att = [];
        foreach (['J', 'N'] as $shift_c) {
            $future_att[$shift_c] = [];
            if (!isset($future_att_raw[$shift_c])) continue;
            
            $dates_with_status = array_keys($future_att_raw[$shift_c]);
            usort($dates_with_status, 'strcmp');
            
            for ($i = 0; $i < count($dates_with_status); $i++) {
                $d1 = $dates_with_status[$i];
                $stat = $future_att_raw[$shift_c][$d1];
                $future_att[$shift_c][$d1] = $stat;
                
                for ($j = $i + 1; $j < count($dates_with_status); $j++) {
                    $d2 = $dates_with_status[$j];
                    $stat2 = $future_att_raw[$shift_c][$d2];
                    $diff_days = round((strtotime($d2) - strtotime($d1)) / 86400);
                    
                    if ($diff_days > 0 && $diff_days <= 6) {
                        if ($stat === $stat2) {
                            for ($fill = 1; $fill < $diff_days; $fill++) {
                                $d_fill = date('Y-m-d', strtotime($d1 . " +$fill days"));
                                $future_att[$shift_c][$d_fill] = $stat;
                            }
                            break;
                        }
                    } else {
    ...
    case 'init_site_period':
        ...
        if (empty($subs)) {
            ...
            elseif ($site_id === 'site_itc') {
                $comp_suffix = substr(preg_replace('/[^a-z0-9]/', '', strtolower($company_id ?? '')), 0, 12);
                $subs = [
                    ['id' => 'itc_tenue_' . $comp_suffix],
                    ['id' => 'itc_costume_' . $comp_suffix],
                    ['id' => 'itc_ots_' . $comp_suffix],
                    ['id' => 'itc_special_' . $comp_suffix]
                ];
            }
        }
                ...
                if ($stype === 'Nuit' || $stype === 'Jour') {
                    ...
                    foreach ($datesList as $ds) {
                        ...
                        if ($isSpecial) {
                            ...
                            $adminDays = $profile['admin_schedule_days'] ?? [6, 7];
                            $adminDays = array_map(function($d) { return $d == 0 ? 7 : $d; }, $adminDays);
                            if (in_array($jsDay, $adminDays) || in_array((string)$jsDay, $adminDays)) {
                                $skip = true;
                            }
                        }
                        ...
                        if (!empty($permanent_supps)) {
                            $daySupps = [];
                            if (isset($permanent_supps[0])) { // Legacy simple array: [1, 2, 5]
                            ...
                            foreach ($daySupps as $shiftKey => $codeToInsert) {
                                $stmtAtt->execute([$agent['id'], $ds, $shiftKey, $codeToInsert, $company_id, $serviceKey, $period]);
                                ...
                                if (strpos($codeToInsert, 'Suppl|') === 0) {
                                    ...
                                    if (isset($parts[1])) {
                                        ...
                                        $sqlite->prepare('DELETE FROM supplementaires_externes WHERE agent_id = ? AND date_supp = ? AND site_destination_id = ? AND periode = ? AND vacation = ?')
                                               ->execute([$agent['id'], $ds, $dest_id, $period, $shiftKey === 'SJ' ? 'Jour' : ($shiftKey === 'SN' ? 'Nuit' : 'Spécial')]);
                                        ...
                                        $sqlite->prepare('INSERT INTO supplementaires_externes (company_id, agent_id, site_origine_id, site_destination_id, date_supp, vacation, periode, agent_remplace) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
                                               ->execute([$company_id, $agent['id'], $site_id, $dest_id, $ds, $shiftKey === 'SJ' ? 'Jour' : ($shiftKey === 'SN' ? 'Nuit' : 'Spécial'), $period, '']);
                                    }
                                }
                            }
                        }
                    ...
                    foreach ($datesList as $idx => $ds) {
                        ...
                        if (!empty($permanent_supps)) {
                            $daySupps = [];
                            if (isset($permanent_supps[0])) { // Legacy simple array
                                if (in_array($jsDay, $permanent_supps) || in_array((string)$jsDay, $permanent_supps)) {
                                    $daySupps['S'] = '1';
                                }
                            } else {
    ...
    case 'add_external_supp':
        ...
        // Récupération de la fonction de l'agent remplacé pour le Dashboard en temps réel
        $rep_func = '';
        if ($agent_remplace) {
            $stmtRep = $sqlite->prepare("SELECT `function` FROM agents WHERE id = ?");
            $stmtRep->execute([$agent_remplace]);
            $rep_func = $stmtRep->fetchColumn() ?: '';
        }
        ...
        for ($i = 0; $i < $days; $i++) {
            ...
            foreach ($shifts_to_insert as $sc) {
                ...
                // Format attendu par le Dashboard: Suppl|dest|agent|motif|rep_func
                $status_string = "Suppl|$site_destination_id|$agent_remplace|$motif|$rep_func";
                
                $sqlite->prepare('INSERT INTO attendance (agent_id, date, shift_code, status, company_id, service_id, period) VALUES (?, ?, ?, ?, ?, ?, ?)')
                       ->execute([$agent_id, $current_date, $sc, $status_string, $company_id, $serviceKey, $period]);
            }
