      ...
      {!selectedKpiAgent ? (
        ...
        Object.keys(attByDate).forEach(date => {
            ...
            if (spAtt) {
                ...
                let gainSp = agentBaseForSp / 30; // Scénario C par défaut
                ...
                if (spAtt.status.startsWith('Suppl|')) {
                    ...
                    if (replacedBase) {
                        const agentDaily = agentBaseForSp / 30;
                        const replacedDaily = replacedBase / 30;
        ...
        if (datesList.length > 30) {
            ...
            if (entrantSortantCount > 0) {
                ...
                // Si on gérait des changements de status, on pourrait ajuster absenceDaysOld/New mais simplifions ici
                if (scObj) {
        ...
        if (selectedKpiAgent.profile_data?.special_service) {
            // Build an attMap exactly like DashboardTable to guarantee identical deduplication behavior
            const attMap = {};
            (selectedKpiAgent.attendance || []).forEach(att => {
        ...
        if (scObj) {
            ...
            prorataBase = Math.round(baseSalaryOld * (regularDaysOld / 30)) + Math.round(baseSalaryNew * (regularDaysNew / 30));
            deductions = Math.round((baseSalaryOld / 30) * absenceDaysOld) + Math.round((baseSalaryNew / 30) * absenceDaysNew);
            costBonus = Math.round(cost_countOld * (costBase / 30)) + Math.round(cost_countNew * (costBase / 30));
            ...
            Object.keys(dynamicFuncCounts).forEach(fcode => {
                ...
                dynamicBonus += Math.round(cOld * (fBase / 30)) + Math.round(cNew * (fBase / 30));
            });
        } else {
            ...
            prorataBase = Math.round(baseSalary * (regularDays / 30));
            const dailyRate = baseSalary / 30;
            deductions = Math.round(dailyRate * absenceDays);
            costBonus = Math.round(cost_count * (costBase / 30));
            
            Object.keys(dynamicFuncCounts).forEach(fcode => {
                const fBase = salaryGrid[fcode] || 75000;
                const cCount = dynamicFuncCounts[fcode] || 0;
                dynamicBonus += Math.round(cCount * (fBase / 30));
            });
        }
