...
export default function PaymentMethodModal({ agent, allSalaries = [], onClose, onSubmit }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (paymentMethod === 'BANQUE') {
      ...
      const safeRibCode = cleanRibCode.padStart(5, '0');
      const safeRibAgc = cleanRibAgc.padStart(5, '0');
      const safeRibCompte = cleanRibCompte.padStart(12, '0');
      const safeRibCle = cleanRibCle.padStart(2, '0');
      
      if (safeRibCode.length !== 5 || safeRibAgc.length !== 5 || safeRibCompte.length !== 12 || safeRibCle.length !== 2) {
        setErrorMsg("Le format du RIB est incorrect (5 - 5 - 12 - 2).");
        return;
      }
      ...
      const updatedProfile = {
        ...
        payment_operator: '',
        payment_country: '',
        payment_rib: `${safeRibCode}${safeRibAgc}${safeRibCompte}${safeRibCle}`,
        ...
        {!isEditing ? (
          ...
          {paymentMethod === 'BANQUE' && (
            <>
              ...
              {errorMsg && (
                <div style={{ color: '#ef4444', fontSize: '0.85rem', marginTop: '8px', background: 'rgba(239,68,68,0.1)', padding: '8px', borderRadius: '4px', border: '1px solid rgba(239,68,68,0.3)' }}>
                  ⚠️ {errorMsg}
                </div>
              )}
            </>
