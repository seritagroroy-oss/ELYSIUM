...
switch ($action) {
    ...
    case 'apply_mutation':
        ...
        $stmtFuture = $sqlite->prepare("SELECT date, shift_code, status FROM attendance WHERE agent_id = ? AND period = ? AND status IN ('A', 'P', 'MAP', 'CP', 'AT')");
        ...
        while ($cursor <= $end_time) {
            ...
            if ($cursor < $start_time) {
                ...
                if ($stype === 'Nuit') {
                    ...
                    $batchRows[] = [$new_agent_id, $date_str, 'J', $saved_any ?? $val, $company_id, $serviceKey, $period];
                    $batchRows[] = [$new_agent_id, $date_str, 'N', $saved_any ?? $val, $company_id, $serviceKey, $period];
                    $cycle_counter++;
                }
            }
