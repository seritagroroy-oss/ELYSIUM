...
switch ($action) {
    case 'get_sites':
        ...
        if ($scope === 'service') {
            $scope = getResolveScope('traitement_pointage');
        }
        ...
        file_put_contents(__DIR__."/dump.json", json_encode($sites);
        break;
    case 'add_site':
        ...
        $row = $stmt->fetch();
        if ($row && $row['c'] > 0) {
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => 'Ce site existe déjà dans ce module']);
            break;
        }
        ...
        file_put_contents(__DIR__."/dump.json", json_encode(['success' => true]);
        break;
    case 'get_site_agents':
        ...
        file_put_contents(__DIR__."/dump.json", json_encode(['success' => true, 'agents' => $agents]);
        break;
    ...
    case 'add_special_site':
        ...
        if ($name === '') {
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => 'Nom requis']);
            break;
        }
        $db = getScopedData($serviceKey);
        ...
        foreach ($db['sites'] as $s) {
            if ($s['id'] === $slug || strtolower($s['name']) === strtolower($name)) {
                file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => 'Un site avec ce nom existe déjà']);
                break 2;
            }
        }
        $db['sites'][] = [
        ...
        saveScopedData($db, $serviceKey);
        file_put_contents(__DIR__."/dump.json", json_encode(['success' => true, 'site_id' => $slug]);
        break;
    ...
    case 'add_subsite':
        if (!hasPermission('dashboard') && !hasPermission('salaries') && !hasPermission('fluctuation')) {
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => 'Accès refusé']);
            break;
        }
        $site_id = $data['site_id'] ?? 0;
        $name = $data['name'] ?? '';
        if (!$site_id || !$name) {
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => 'Données manquantes']);
            break;
        }
        $sqlite = getDb();
        ...
        try {
            ...
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => true, 'id' => $id, 'name' => $name]);
        } catch (Throwable $e) {
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => 'Erreur DB: ' . $e->getMessage()]);
        }
        break;
    case 'rename_site':
        requirePermission('dashboard');
        ...
        break;
    case 'delete_site':
        error_log("DEBUG delete_site: started for site_id = " . ($data['site_id'] ?? 'none'));
        requirePermission('dashboard');
        $site_id = $data['site_id'] ?? '';
        $motif = $data['motif'] ?? '';
        $is_billed = isset($data['is_billed']) ? (int)$data['is_billed'] : 1;
        if (!$site_id) {
            error_log("DEBUG delete_site: missing site_id");
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => 'Site manquant']);
            break;
        }
        $sqlite = getDb();
        ...
        if (empty($subsites) && $motif) {
            $stmtInsert = $sqlite->prepare("INSERT INTO contract_ruptures 
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmtInsert->execute([
                $company_id, $site_id, 'Aucune zone', $site_name, $motif, date('Y-m-d'), 0, 0, '[]', date('Y-m-d H:i:s'), $user, $is_billed
            ]);
        }
        ...
        foreach ($subsites as $sub) {
            if ($motif) {
                $stmtInsert = $sqlite->prepare("INSERT INTO contract_ruptures 
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmtInsert->execute([
                    $company_id, $sub['id'], $sub['name'], $site_name, $motif, date('Y-m-d'), 0, 0, '[]', date('Y-m-d H:i:s'), $user, $is_billed
                ]);
            }
    ...
    case 'delete_subsite':
        ...
        $subsite_id = $data['subsite_id'] ?? '';
        if (!$subsite_id) {
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => 'Sous-site manquant']);
            break;
        }

        if (strpos((string) $subsite_id, 'mutated_') === 0) {
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => 'Sous-site temporaire non supprimable']);
            break;
        }
    ...
    case 'get_site_data':
        ...
        if ($lockData && !empty($lockData['password_hash'])) {
            if (empty($_SESSION['unlocked_periods'][$company_id][$period])) {
                file_put_contents(__DIR__."/dump.json", json_encode(['locked' => true, 'period' => $period]);
                exit;
            }
        }
        ...
        if ($site_id !== null && $site_id !== '' && $serviceKey) {
            ...
            if ($site) {
                ...
                foreach ($subsites_rows as $sub) {
                    ...
                    foreach ($agents_rows as $agent) {
                        ...
                        if (strpos($agent['id'], 'ag_') === 0) {
                            $origin_stats = resolve_agent_origin_stats($sqlite, $agent['id'], $period);
                            $agent['origin_absences'] = $origin_stats['origin_absences'];
                            $agent['days_consumed_by_origin'] = $origin_stats['days_consumed_by_origin'];
                            $agent['is_clone'] = true;
                        }

                        if ($agent['name'] === 'OSLO') {
                            $agent['name'] = 'OSLO [oA:' . ($agent['origin_absences'] ?? 'X') . ' oB:' . ($agent['days_consumed_by_origin'] ?? 'X') . ' C:' . (isset($agent['is_clone']) ? '1' : '0') . ']';
                        }
                ...
                $suppl_conditions = [];
                $suppl_params = [];
                $suppl_conditions[] = "(a.status = ? OR a.status LIKE ?)";
               ...
               ");
                ...
                foreach ($mutated_rows as $agent) {
                    ...
                    if ($orig_site && !$is_scheduled_here) {
                        ...
                        if (strpos($agent['id'], 'ag_') === 0) {
                            $origin_stats = resolve_agent_origin_stats($sqlite, $agent['id'], $period);
                            $mutated_agent['origin_absences'] = $origin_stats['origin_absences'];
                            $mutated_agent['days_consumed_by_origin'] = $origin_stats['days_consumed_by_origin'];
                        } else {
                            $mutated_agent['origin_absences'] = 0;
                            $mutated_agent['days_consumed_by_origin'] = 0;
                        }

                        if ($mutated_agent['name'] === 'OSLO') {
                            $mutated_agent['name'] = 'OSLO (Mut) [oA:' . ($mutated_agent['origin_absences'] ?? 'X') . ' oB:' . ($mutated_agent['days_consumed_by_origin'] ?? 'X') . ']';
                        }
        ...
        file_put_contents(__DIR__."/dump.json", json_encode($site_data);
        break;
    case 'add_agent':
        ...
        $debut_date_raw = $data['debutDate'] ?? ''; // Date de début pour agents TP (Définir une date)
        ...
        if ($stmtBlacklist->fetch()) {
             file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => "Impossible d'ajouter cet agent : il figure sur la liste noire de l'entreprise."]);
             break;
        }
        ...
        $is_entrant = isset($data['isEntrant']) && $data['isEntrant'] ? true : false;
        ...
        if ($period !== '') {
            ...
            // ─── Appliquer la date de début (debut_date) pour les agents TP ──────────────────
            // Si debut_date est fourni, remplacer les jours AVANT cette date par ENTRANT
            // et supprimer les présences (1, R) pour les jours AVANT la date de début
            $debut_date = '';
            if (!empty($debut_date_raw)) {
                $dObj = DateTime::createFromFormat('Y-m-d', $debut_date_raw);
                if (!$dObj) $dObj = DateTime::createFromFormat('d/m/Y', $debut_date_raw);
                if (!$dObj) $dObj = DateTime::createFromFormat('d-m-Y', $debut_date_raw);
                if ($dObj) $debut_date = $dObj->format('Y-m-d');
            } elseif ($is_entrant && !empty($entrant_date_raw)) {
                $dObj = DateTime::createFromFormat('Y-m-d', $entrant_date_raw);
                if (!$dObj) $dObj = DateTime::createFromFormat('d/m/Y', $entrant_date_raw);
                if (!$dObj) $dObj = DateTime::createFromFormat('d-m-Y', $entrant_date_raw);
                if ($dObj) $debut_date = $dObj->format('Y-m-d');
            }

            if ($debut_date && isset($db['attendance'][$period][$new_agent_id])) {
                $targetShiftForEntrant = ($shift_type === 'Nuit' || $shift_type === 'N') ? 'N' : 'J';
                $statusToApply = ($is_entrant && !empty($entrant_motif)) ? $entrant_motif : 'NON_PRESENT';
                // Pour chaque date dans les présences générées, si la date est AVANT debut_date,
                // on remplace par le statut approprié (NON_PRESENT, ENTRANT, ou REINTEGRATION)
                $allDatesInAttendance = array_unique(array_merge(
                    array_keys($db['attendance'][$period][$new_agent_id]['J'] ?? []),
                    array_keys($db['attendance'][$period][$new_agent_id]['N'] ?? [])
                ));
                foreach ($allDatesInAttendance as $attDate) {
                    if ($attDate < $debut_date) {
                        // Remplacer par le statut souhaité sur le shift principal
                        $db['attendance'][$period][$new_agent_id][$targetShiftForEntrant][$attDate] = $statusToApply;
                        // Supprimer de l'autre shift si présent
                        $otherShift = $targetShiftForEntrant === 'J' ? 'N' : 'J';
                        unset($db['attendance'][$period][$new_agent_id][$otherShift][$attDate]);
                    }
                }
                // Mettre à jour la hire_date de l'agent
                $sqlite->prepare("UPDATE agents SET hire_date = ? WHERE id = ?")
                       ->execute([$debut_date, $new_agent_id]);
            }
        ...
        saveScopedData($db, $serviceKey);
        file_put_contents(__DIR__."/dump.json", json_encode(['success' => true, 'agent_id' => $new_agent_id]);
        break;
    case 'delete_agent':
        $agent_id = $data['agent_id'] ?? '';
        if (!$agent_id) {
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => 'Agent invalide']);
            break;
        }
        $sqlite = getDb();
    ...
    case 'update_agent_profile':
        ...
        if ($period) {
            ...
            if ($snapRow && $snapRow['snapshot']) {
                $snapshotData = json_decode($snapRow['snapshot'], true);
                $updated = false;
                if (is_array($snapshotData)) {
                    foreach ($snapshotData as &$agentData) {
                        if (isset($agentData['id']) && $agentData['id'] === $agent_id) {
                            $agentData['profile_data'] = $profile_data;
                            $updated = true;
                            break;
                        }
                    }
            ...
            // Mettre à jour l'archive si elle existe (mois clôturé)
            $archive_id = "payroll_" . $period;
            $stmtArch = $sqlite->prepare("SELECT data FROM archives WHERE company_id = ? AND id = ?");
            $stmtArch->execute([$company_id, $archive_id]);
            $archiveRow = $stmtArch->fetch();
            if ($archiveRow && $archiveRow['data']) {
                $archiveData = json_decode($archiveRow['data'], true);
                ...
                if ($updatedArchive) {
                    $updateArchStmt = $sqlite->prepare("UPDATE archives SET data = ? WHERE company_id = ? AND id = ?");
                    $updateArchStmt->execute([json_encode($archiveData, JSON_UNESCAPED_UNICODE), $company_id, $archive_id]);
                }
            }
        }
        ...
        file_put_contents(__DIR__."/dump.json", json_encode(['success' => true]);
        break;
    case 'update_agent_admin_schedule':
        $agent_id = $data['agent_id'] ?? '';
        $admin_schedule_days = $data['admin_schedule_days'] ?? [];
        ...
        if (isPayrollPeriodLocked($sqlite, $company_id, $period)) {
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => 'Période verrouillée']);
            break;
        }

        // 1. Récupérer l'agent actuel
        $stmt = $sqlite->prepare("SELECT * FROM agents WHERE id = ?");
        $stmt->execute([$agent_id]);
        $agentRow = $stmt->fetch();
        if (!$agentRow) {
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => 'Agent introuvable']);
            break;
        }

        $profile = json_decode($agentRow['profile_data'] ?? '{}', true);
        $profile['admin_schedule'] = count($admin_schedule_days) > 0;
        $profile['admin_schedule_days'] = $admin_schedule_days;
        
        $sqlite->prepare("UPDATE agents SET profile_data = ? WHERE id = ?")
               ->execute([json_encode($profile), $agent_id]);

        // 2. Régénérer les présences pour le cycle actuel
        ...
        applyShiftDefaultsForPeriod($db, $agent_id, $period, $agentRow['shift_type'], $profile['admin_schedule'], $is_special, $special_days, $admin_schedule_days);
        ...
        file_put_contents(__DIR__."/dump.json", json_encode(['success' => true]);
        break;
    case 'update_agent_special_service':
        $agent_id = $data['agent_id'] ?? '';
        $special_service_base = (int)($data['specialServiceBase'] ?? 12);
        $special_service_days = $data['specialServiceDays'] ?? [];
        $is_entrant = (bool)($data['isEntrant'] ?? false);
        $entrant_date = $data['entrantDate'] ?? '';
        $is_debut = (bool)($data['isDebut'] ?? false);
        $debut_date_raw = $data['debutDate'] ?? '';
        $period = $data['period'] ?? date('Y-m');
        $serviceKey = $_SESSION['service_id'] ?? null;
        ...
        $profile = json_decode($agentRow['profile_data'] ?? '{}', true);
        $profile['special_service'] = true;
        $profile['special_service_base'] = $special_service_base;
        $profile['special_service_days'] = $special_service_days;

        // 2. Mettre à jour hire_date et profile_data
        $new_hire_date = null;
        if ($is_entrant && $entrant_date) {
            $new_hire_date = $entrant_date;
        } elseif ($is_debut && $debut_date_raw) {
            $new_hire_date = $debut_date_raw;
        }
        
        $sqlite->prepare("UPDATE agents SET profile_data = ?, hire_date = ? WHERE id = ?")
               ->execute([json_encode($profile), $new_hire_date, $agent_id]);

        // 3. Régénérer les présences pour le cycle actuel
        // Supprimer toutes les présences de cet agent pour ce cycle dans SQLite
        $sqlite->prepare("DELETE FROM attendance WHERE agent_id = ? AND period = ?")->execute([$agent_id, $period]);

        // Générer le planning par défaut dans $db
        $db = getScopedData($serviceKey);
        applyShiftDefaultsForPeriod($db, $agent_id, $period, $agentRow['shift_type'], false, true, $special_service_days);

        // Si une date de début/d'entrée a été définie, on applique les statuts d'inactivité avant cette date dans $db
        if ($new_hire_date) {
            $dObj = DateTime::createFromFormat('Y-m-d', $new_hire_date);
            if (!$dObj) $dObj = DateTime::createFromFormat('d/m/Y', $new_hire_date);
            if (!$dObj) $dObj = DateTime::createFromFormat('d-m-Y', $new_hire_date);
            $parsed_hire_date = $dObj ? $dObj->format('Y-m-d') : $new_hire_date;

            $statusToApply = $is_entrant ? 'ENTRANT' : 'NON_PRESENT';
            $targetShift = ($agentRow['shift_type'] === 'Nuit' || $agentRow['shift_type'] === 'N') ? 'N' : 'J';

            $settingsRow = getServiceDataSql($serviceKey, 'settings', ['cycle_start' => 21, 'cycle_end' => 20]);
            $dates = getPeriodDates($period, (int)($settingsRow['cycle_start'] ?? 21), (int)($settingsRow['cycle_end'] ?? 20));

            foreach ($dates as $d) {
                if ($d < $parsed_hire_date) {
                    $db['attendance'][$period][$agent_id][$targetShift][$d] = $statusToApply;
                    // Supprimer du shift opposé si existant
                    $otherShift = $targetShift === 'J' ? 'N' : 'J';
                    unset($db['attendance'][$period][$agent_id][$otherShift][$d]);
                }
            }
        }

        // Écrire toutes les présences finales de $db dans SQLite
        if (isset($db['attendance'][$period][$agent_id])) {
            $stmtAtt = $sqlite->prepare('INSERT INTO attendance (agent_id, date, shift_code, status, company_id, service_id, period) VALUES (?, ?, ?, ?, ?, ?, ?)');
            foreach ($db['attendance'][$period][$agent_id] as $shift_code => $days) {
                foreach ($days as $date => $status) {
            ...
            if ($field === 'shift_type' && $period) {
                ...
                if ($agentProfile && $agentProfile['profile_data']) {
                    ...
                    if (!empty($profile['admin_schedule'])) {
                        $is_admin = true;
                        $admin_days = $profile['admin_schedule_days'] ?? [];
                    }
                ...
                applyShiftDefaultsForPeriod($db, $agent_id, $period, $value, $is_admin, $is_special, $special_days, $admin_days);
    ...
    case 'get_functions':
        ...
        file_put_contents(__DIR__."/dump.json", json_encode(['success' => true, 'functions' => $functions]);
        break;
    ...
    case 'archive_all_sites':
        ...
        if (!$period || !preg_match('/^\d{4}-\d{2}$/', $period)) {
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => 'Période invalide: ' . $period]);
            break;
        }
        ...
        file_put_contents(__DIR__."/dump.json", json_encode(['success' => true, 'sites_count' => count($snapshot_sites)]);
        break;
    case 'get_archives':
        $sqlite = getDb();
        $company_id = $_SESSION['company_id'] ?? 'comp_default_1';
        ...
        file_put_contents(__DIR__."/dump.json", json_encode($archives);
        break;
    case 'get_archive_detail':
        $id = $_GET['id'] ?? '';
        if (!$id) {
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => 'ID manquant']);
            break;
        }
        ...
        if ($result && isset($result['data'])) {
            ...
            if (isset($db['archives'][$id])) {
                file_put_contents(__DIR__."/dump.json", json_encode($db['archives'][$id]);
            } else {
                file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => 'Archive introuvable']);
            }
        }
        break;
    case 'delete_archive':
    ...
    case 'get_closure_alerts':
        ...
        file_put_contents(__DIR__."/dump.json", json_encode(['success' => true, 'alerts' => $alerts]);
        break;
    ...
    case 'clear_site_mutations':
        ...
        if (!$site_id || !$period) {
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => 'Données incomplètes']);
            break;
        }
        ...
        if (!$target_site) {
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => 'Site introuvable']);
            break;
        }
    ...
    case 'get_agent_schedules':
        ...
        if (!$agent_id) {
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => 'Agent manquant']);
            break;
        }
        $sqlite = getDb();
        $stmt = $sqlite->prepare("SELECT day_of_week, target_site_id, target_subsite_id FROM agent_schedules WHERE agent_id = ?");
        $stmt->execute([$agent_id]);
        file_put_contents(__DIR__."/dump.json", json_encode(['success' => true, 'data' => $stmt->fetchAll(PDO::FETCH_ASSOC)]);
        break;
    ...
    case 'update_agent_schedules':
        ...
        $stmt_all_sites = $sqlite->prepare("SELECT id, name FROM sites WHERE company_id = ?");
        $stmt_all_sites->execute([$company_id]);
        $all_sites = $stmt_all_sites->fetchAll();
        $site_name_map = [];
        ...
        if (!$force_apply) {
            ...
            if (count($conflicts) > 0) {
                file_put_contents(__DIR__."/dump.json", json_encode(['success' => true, 'has_conflicts' => true, 'conflicts' => $conflicts]);
                break;
            }
        }
        ...
        file_put_contents(__DIR__."/dump.json", json_encode(['success' => true]);
            ...
            $sqlite->rollBack();
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'message' => $e->getMessage()]);
        }
        break;
    ...
    case 'get_lost_sites':
        ...
        file_put_contents(__DIR__."/dump.json", json_encode(['success' => true, 'lost_sites' => $lost_sites, 'sites' => $lost_sites]);
        break;
    ...
    case 'move_agent_zone':
        ...
        if (empty($agent_id) || empty($new_subsite_id)) {
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'error' => 'Paramètres manquants']);
            exit;
        }
        ...
        try {
            ...
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'error' => $e->getMessage()]);
        }
        break;
    ...
    case 'verify_period_lock':
        ...
        if ($lock && password_verify($password, $lock['password_hash'])) {
            ...
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'error' => 'Mot de passe incorrect']);
        }
        break;
    ...
    case 'toggle_period_lock':
        ...
        if ($action_type === 'unlock') {
            ...
            $stmt->execute([$company_id, $period]);
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => true, 'message' => 'Mois déverrouillé avec succès.']);
        } else {
            if (!$password) {
                file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'error' => 'Mot de passe requis pour verrouiller.']);
                exit;
            }
            ...
            $_SESSION['unlocked_periods'][$company_id][$period] = true;
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => true, 'message' => 'Mois verrouillé avec succès.']);
        }
        break;

    case 'toggle_permanent_supplement':
        $sqlite = getDb();
        // Le corps de la requête contient l'ID de l'agent et les jours
        // Si les données sont envoyées en JSON, elles sont dans $data (récupéré au début du fichier)
        $agent_id = $data['agent_id'] ?? $_POST['agent_id'] ?? null;
        $days = isset($data['days']) ? $data['days'] : (isset($_POST['days']) ? $_POST['days'] : []);
        ...
        if (!$agent_id) {
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'error' => 'Agent ID manquant']);
            exit;
        }
        ...
        if ($agent) {
            ...
            $profile['permanent_supps'] = $days;
            $stmtUpd = $sqlite->prepare("UPDATE agents SET profile_data = ? WHERE id = ?");
            $stmtUpd->execute([json_encode($profile), $agent_id]);
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => true]);
        } else {
            file_put_contents(__DIR__."/dump.json", json_encode(['success' => false, 'error' => 'Agent introuvable']);
        }
        break;
}
