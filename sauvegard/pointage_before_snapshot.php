...
switch ($action) {
    ...
    case 'get_dashboard_init':
        ...
        // Settings du cycle pour déterminer la fin de la période (pour exclure les agents futurs)
        ...
        $datesForPeriod = getPeriodDates($period, $start_day, $end_day);
        $period_end_date = end($datesForPeriod);
        ...
        // EAGER LOADING: Fetch all subsites at once
            $allSubsitesBySite = [];
            try {
                $stmtAllSub = $sqlite->prepare("SELECT * FROM subsites");
                $stmtAllSub->execute();
            ...
            try {
                $stmtAgCountAll = $sqlite->prepare("SELECT subsite_id, COUNT(*) as cnt FROM agents WHERE company_id = ? AND (archived_period IS NULL OR archived_period = '' OR archived_period >= ?) AND (hire_date IS NULL OR hire_date <= ?) GROUP BY subsite_id");
                $stmtAgCountAll->execute([$company_id, $period, $period_end_date]);
            ...
            if ($site_id !== null && $site_id !== '') {
                ...
                if ($site) {
                    ...
                    if (!empty($subsite_ids)) {
                        ...
                        // 1. Fetch all agents
                        $stmt_ag = $sqlite->prepare("SELECT * FROM agents WHERE subsite_id IN ($inClause) AND company_id = ? AND (archived_period IS NULL OR archived_period = '' OR archived_period >= ?) AND (hire_date IS NULL OR hire_date <= ?) ORDER BY created_at ASC");
                        $stmt_ag->execute([$company_id, $period, $period_end_date]);
                   ...
                   ");
                    ...
                    $params = [$period, $like_m, $like_ext, $like_rel];
        ...
        try {
            $stmtGlobal = $sqlite->prepare("
                ...
                AND (a.archived_period IS NULL OR a.archived_period >= ?)
                AND (a.hire_date IS NULL OR a.hire_date <= ?)
                AND a.id NOT LIKE 'agt_sp_%'
            ");
            $stmtGlobal->execute([$company_id, $serviceKey, $serviceKey, $period, $period_end_date]);
                ...
                if (!empty($subIds)) {
                    ...
                    foreach ($mutatedRows as $mut) {
                        if (!in_array($mut['id'], $existing_agent_ids)) {
                            if (empty($subsitesMap)) {
                                $subsitesMap['default_' . $site['id']] = ['id' => 'default_' . $site['id'], 'name' => 'Zone Principale', 'agents' => []];
                                $subIds[] = 'default_' . $site['id'];
                            }
                            $dest_subsite = $subIds[0];
                            $mut['subsite_id'] = $dest_subsite;
                            $agentRows[] = $mut;
    ...
    case 'archive_pointage':
        ...
        if ($user_id) {
            $stmtU = $sqlite->prepare("SELECT first_name, last_name FROM users WHERE email = ?");
