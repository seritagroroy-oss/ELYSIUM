...
export function useAgentPointage({
  ...
  const handleCellClick = async (agentId, dateKey, shiftCode, currentStatus, forcedStatus = null, agentObj = null) => {
    ...
    // ✅ Verrou silencieux : mois publié lu depuis la photo figée
    // Aucun toast, aucun flash visuel — la cellule ne réagit tout simplement pas
    if (isLockedByArchive) return;
    ...
    if (forcedStatus !== null) {
      ...
      if (isExtra) {
        ...
        if (functionModes[agentId]) {
          ...
          const isRotative = ['24h', '48h', '72h'].includes(agentShiftType?.toLowerCase());
  ...
  const executeSegmentRepos = (agent, daysOfWeek, segment) => {
    ...
    const stLowerCase = segment.type?.toLowerCase() || '';
    const isRotative = ['24h', '48h', '72h'].includes(stLowerCase);
    const isNight = ['nuit', '12 n', '12h n'].includes(stLowerCase);
    const scList = isRotative ? ['J', 'N'] : (isNight ? ['N'] : ['J']);
    ...
    datesList.forEach(d => {
      ...
      // Protéger les jours inactifs (avant embauche / après départ) uniquement si aucun statut manuel n'a été forcé
      const currentSt = agent.attendance?.find(a => a.date === dk && a.shift_code === scList[0])?.status;
      if (agent.hire_date && dk < agent.hire_date) {
        if (!currentSt || currentSt === 'ENTRANT' || currentSt === 'NON_PRESENT') return;
      }
      if (agent.exit_date && dk > agent.exit_date) {
        if (!currentSt || ['ABANDON', 'DEMISSION', 'RETIRE', 'LICENCIE', 'LICENCIE_ADMIN', 'FIN_CONTRAT'].includes(currentSt) || currentSt.startsWith('SORTANT_')) return;
      }
  ...
  const executeAssignRepos = async () => {
    ...
    // 1. Mise à jour optimiste locale pour une réactivité instantanée
    setSiteData(prevData => {
      return prevData.map(subsite => {
        return {
          ...subsite,
          agents: subsite.agents.map(agent => {
            ...
            updates.forEach(upd => {
              if (String(upd.agent_id) === String(agent.id)) {
                agentModified = true;
                const idx = updatedAttendance.findIndex(a => a.date === upd.date && a.shift_code === upd.shift_code);
                if (idx > -1) {
                  if (upd.status === '') {
                    updatedAttendance.splice(idx, 1);
                  } else {
                    updatedAttendance[idx].status = upd.status;
                  }
                } else if (upd.status !== '') {
                  updatedAttendance.push({
                    date: upd.date,
                    shift_code: upd.shift_code,
                    status: upd.status
                  });
                }
              }
    ...
    // 2. Appel API en arrière-plan (sans bloquer l'UI avec setLoading)
    try {
      ...
      if (!res.success) {
        ...
        loadSiteData(); // Recharge seulement en cas d'erreur pour corriger l'état
      }
    } catch (e) {
      console.error(e);
      loadSiteData(); // Recharge en cas d'erreur réseau
    }
  };
