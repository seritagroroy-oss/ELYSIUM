  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        boxShadow: '0 25px 60px rgba(0,0,0,0.7), 0 0 50px rgba(245,158,11,0.1), inset 0 1px 0 rgba(255,255,255,0.05)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '28px' }}>
          <div style={{
            ...
            fontSize: '1.6rem', boxShadow: '0 6px 20px rgba(245,158,11,0.15)'
          }}>⚠️</div>
          <div>
            <h3 style={{ margin: 0, fontSize: '1.2rem', fontWeight: 800, color: '#fff', letterSpacing: '-0.02em' }}>Absence Injustifiée (A)</h3>
            <p style={{ margin: '3px 0 0 0', fontSize: '0.85rem', color: 'rgba(255,255,255,0.4)' }}>
              Agent : <span style={{ color: '#fbbf24', fontWeight: 700 }}>{agentName}</span>
            </p>
          </div>
        </div>
        ...
        <div style={{ marginBottom: '24px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', marginBottom: '16px', background: 'rgba(0,0,0,0.3)', padding: '6px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
              ...
              <button type="button" onClick={() => { localStorage.setItem('absence_selection_mode', 'extended'); setAbsenceNavOffset(0); }} style={{ flex: 1, padding: '8px', borderRadius: '8px', border: 'none', background: (localStorage.getItem('absence_selection_mode') || 'nav') === 'extended' ? 'rgba(239,68,68,0.2)' : 'transparent', color: (localStorage.getItem('absence_selection_mode') || 'nav') === 'extended' ? '#f87171' : 'rgba(255,255,255,0.5)', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s' }}>Vue 45 jours</button>
              <button type="button" onClick={() => { localStorage.setItem('absence_selection_mode', 'manual'); setAbsenceNavOffset(0); onManualDurationChange('3'); }} style={{ flex: 1, padding: '8px', borderRadius: '8px', border: 'none', background: (localStorage.getItem('absence_selection_mode') || 'nav') === 'manual' ? 'rgba(239,68,68,0.2)' : 'transparent', color: (localStorage.getItem('absence_selection_mode') || 'nav') === 'manual' ? '#f87171' : 'rgba(255,255,255,0.5)', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s' }}>Saisie Durée</button>
            </div>

            <label style={{ display: 'block', marginBottom: '16px', color: 'rgba(255,255,255,0.7)', fontSize: '0.95rem', textAlign: 'center' }}>
              {(() => {
                const mode = localStorage.getItem('absence_selection_mode') || 'nav';
              ...
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
                ...
                <span style={{ fontSize: '0.9rem', fontWeight: 'bold', color: 'rgba(255,255,255,0.9)', minWidth: '150px', textAlign: 'center' }}>
                  {(() => {
                    ...
                    let start = new Date(y, m - 2 + absenceNavOffset, cycleStart);
                    let end = new Date(y, m - 1 + absenceNavOffset, cycleStart - 1);
                    return `${start.toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })} - ${end.toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}`;
                  })()}
                </span>
                <button type="button" onClick={() => setAbsenceNavOffset(o => o + 1)} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', color: '#fff', padding: '6px 12px', borderRadius: '6px', cursor: 'pointer', transition: 'all 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.2)'} onMouseLeave={e => e.currentTarget.style.background = 'rgba(255,255,255,0.1)'}>➡️</button>
                <button type="button" onClick={() => setAbsenceNavOffset(0)} style={{ background: 'none', border: '1px solid rgba(255,255,255,0.2)', color: 'rgba(255,255,255,0.6)', padding: '6px 10px', borderRadius: '6px', cursor: 'pointer', fontSize: '0.8rem', marginLeft: '4px' }}>Aujourd'hui</button>
              </div>
            )}
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', justifyContent: 'center' }}>
              {(() => {
                ...
                if (mode === 'nav') {
                  startD.setMonth(startD.getMonth() + absenceNavOffset);
            ...
            {(localStorage.getItem('absence_selection_mode') || 'nav') === 'manual' && startDate && (
          ...
          {startDate && endDate && startDate <= endDate && (() => {
            ...
            return (
              <div style={{ background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.15)', borderRadius: '8px', padding: '10px 14px', marginBottom: '24px', fontSize: '0.85rem', color: 'rgba(255,255,255,0.55)', textAlign: 'center' }}>
                🔴 <strong style={{ color: '#f87171' }}>{count} jour{count > 1 ? 's' : ''}</strong> d'absence seront enregistrés
              </div>
            );
          })()}
