    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '28px' }}>
          ...
          <div>
            <h3 style={{ margin: 0, fontSize: '1.2rem', fontWeight: 800, color: '#fff', letterSpacing: '-0.02em' }}>Maladie (M)</h3>
        ...
        <form onSubmit={async (e) => { 
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', marginBottom: '16px', background: 'rgba(0,0,0,0.3)', padding: '6px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
              <button type="button" onClick={() => { localStorage.setItem('maladie_selection_mode', 'nav'); setMaladieNavOffset(0); }} style={{ flex: 1, padding: '8px', borderRadius: '8px', border: 'none', background: (localStorage.getItem('maladie_selection_mode') || 'nav') === 'nav' ? 'rgba(59,130,246,0.2)' : 'transparent', color: (localStorage.getItem('maladie_selection_mode') || 'nav') === 'nav' ? '#60a5fa' : 'rgba(255,255,255,0.5)', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s' }}>Mois par mois</button>
              <button type="button" onClick={() => { localStorage.setItem('maladie_selection_mode', 'extended'); setMaladieNavOffset(0); }} style={{ flex: 1, padding: '8px', borderRadius: '8px', border: 'none', background: (localStorage.getItem('maladie_selection_mode') || 'nav') === 'extended' ? 'rgba(59,130,246,0.2)' : 'transparent', color: (localStorage.getItem('maladie_selection_mode') || 'nav') === 'extended' ? '#60a5fa' : 'rgba(255,255,255,0.5)', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s' }}>Vue 45 jours</button>
              <button type="button" onClick={() => { localStorage.setItem('maladie_selection_mode', 'manual'); setMaladieNavOffset(0); onManualDurationChange('3'); }} style={{ flex: 1, padding: '8px', borderRadius: '8px', border: 'none', background: (localStorage.getItem('maladie_selection_mode') || 'nav') === 'manual' ? 'rgba(59,130,246,0.2)' : 'transparent', color: (localStorage.getItem('maladie_selection_mode') || 'nav') === 'manual' ? '#60a5fa' : 'rgba(255,255,255,0.5)', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s' }}>Saisie Durée</button>
            </div>

            <label style={{ display: 'block', marginBottom: '16px', color: 'rgba(255,255,255,0.7)', fontSize: '0.95rem', textAlign: 'center' }}>
              {(() => {
                const mode = localStorage.getItem('maladie_selection_mode') || 'nav';
              ...
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
                ...
                <span style={{ fontSize: '0.9rem', fontWeight: 'bold', color: 'rgba(255,255,255,0.9)', minWidth: '150px', textAlign: 'center' }}>
                  {(() => {
                    ...
                    let start = new Date(y, m - 2 + maladieNavOffset, cycleStart);
                    let end = new Date(y, m - 1 + maladieNavOffset, cycleStart - 1);
                    return `${start.toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })} - ${end.toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}`;
                  })()}
                </span>
                <button type="button" onClick={() => setMaladieNavOffset(o => o + 1)} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', color: '#fff', padding: '6px 12px', borderRadius: '6px', cursor: 'pointer', transition: 'all 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.2)'} onMouseLeave={e => e.currentTarget.style.background = 'rgba(255,255,255,0.1)'}>➡️</button>
                <button type="button" onClick={() => setMaladieNavOffset(0)} style={{ background: 'none', border: '1px solid rgba(255,255,255,0.2)', color: 'rgba(255,255,255,0.6)', padding: '6px 10px', borderRadius: '6px', cursor: 'pointer', fontSize: '0.8rem', marginLeft: '4px' }}>Aujourd'hui</button>
              </div>
            )}
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', justifyContent: 'center' }}>
              {(() => {
                ...
                if (mode === 'nav') {
                  startD.setMonth(startD.getMonth() + maladieNavOffset);
                ...
                const getBg = () => {
                  if (isSelected) return 'rgba(59, 130, 246, 0.9)'; // bright blue
                  if (isBetween) return 'rgba(59, 130, 246, 0.25)';  // light blue
                  return 'rgba(255, 255, 255, 0.05)';
                };

                const getColor = () => {
                  if (isSelected) return '#fff';
                  if (isBetween) return '#93c5fd';
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      background: getBg(),
                      color: getColor(),
                      border: isSelected ? '1px solid rgba(255,255,255,0.8)' : (isBetween ? '1px solid rgba(59,130,246,0.5)' : '1px solid rgba(255,255,255,0.1)'),
                      borderRadius: '8px',
                      cursor: 'pointer',
                      fontSize: '0.9rem',
            ...
            {(localStorage.getItem('maladie_selection_mode') || 'nav') === 'manual' && startDate && (
              <div style={{ marginTop: '16px', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '12px', background: 'rgba(255,255,255,0.03)', padding: '12px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.1)' }}>
                ...
                }} style={{ width: '80px', background: 'rgba(0,0,0,0.4)', color: '#fff', border: '1px solid rgba(59,130,246,0.4)', borderRadius: '6px', padding: '8px 10px', fontSize: '1rem', textAlign: 'center', outline: 'none' }} placeholder="Ex: 5" />
              </div>
            )}
          </div>
          ...
          {startDate && endDate && startDate <= endDate && (() => {
            ...
            return (
              <div style={{ background: 'rgba(59,130,246,0.06)', border: '1px solid rgba(59,130,246,0.15)', borderRadius: '8px', padding: '10px 14px', marginBottom: '24px', fontSize: '0.85rem', color: 'rgba(255,255,255,0.55)', textAlign: 'center' }}>
                🤒 <strong style={{ color: '#60a5fa' }}>{count} jour{count > 1 ? 's' : ''}</strong> de maladie seront enregistrés
              </div>
            );
          })()}
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            ...
            <button
              ...
              style={{
                ...
                background: (!startDate || !endDate || isSubmitting) ? 'rgba(59,130,246,0.3)' : 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)',
                ...
                boxShadow: (!startDate || !endDate || isSubmitting) ? 'none' : '0 6px 15px rgba(59,130,246,0.3)',
                transition: 'all 0.2s',
              ...
              onMouseLeave={e => {
                if(startDate && endDate && !isSubmitting) {
                  ...
                  e.currentTarget.style.boxShadow = '0 6px 15px rgba(59,130,246,0.3)';
                }
              }}
            >
              {isSubmitting ? (
              ...
              ) : (!startDate || !endDate) ? 'Sélectionnez une période' : 'Valider la Maladie'}
            </button>
          </div>
        </form>
      </div>
