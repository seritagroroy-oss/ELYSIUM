...
export default function MasseSalariale({ salaries = [], period = '', payrollSettings = {}, pdfAdminTitle, pdfAdminName, headerLeft, headerRight }) {
  ...
  const processedSalaries = salaries.map(agent => {
    ...
    if (agent.profile_data) {
      try {
        ...
        if (pd.payment_method && pd.payment_method !== 'NON DEFINI') paymentMethod = pd.payment_method;
  ...
  const generateExcel = async (agents, title, type = 'virement') => {
    ...
    if (type === 'virement') {
      ...
      sheet.columns = [
        { width: 5 }, { width: 35 }, { width: 35 }, { width: 12 },
      ...
      sheet.columns = [
        { width: 5 }, { width: 40 }, { width: 40 }, { width: 12 }, { width: 20 }
      ];
  ...
  const especeAgents = processedSalaries.filter(a => {
    const pm = (a.paymentMethod || '').trim().toUpperCase();
    return pm === 'ESPECES' || pm === 'ESPECE' || pm === 'AUCUN' || pm === 'NON DEFINI' || pm === 'NON DÉFINI' || pm === '';
  });
  ...
  // Le système doit faire le cumul de TOUS les net du journal de paie, y compris ceux sans mode de paiement (AUCUN)
  const totalGlobal = processedSalaries.reduce((sum, a) => sum + (Number(a.net) || 0), 0);
  ...
  return (
    <div className="glass-panel" style={{ animation: 'slideUp 0.3s ease-out', padding: '30px', position: 'relative' }}>
      <style>{`
        .card-hover { transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); cursor: default; }
        .card-total:hover { box-shadow: 0 15px 40px rgba(56,189,248,0.3) !important; transform: translateY(-5px); border-color: rgba(56,189,248,0.5) !important; }
        .card-virement:hover { box-shadow: 0 12px 24px rgba(56,189,248,0.25) !important; transform: translateY(-5px); border-color: rgba(56,189,248,0.4) !important; }
        .card-espece:hover { box-shadow: 0 12px 24px rgba(250,204,21,0.25) !important; transform: translateY(-5px); border-color: rgba(250,204,21,0.4) !important; }
        .card-mm:hover { box-shadow: 0 12px 24px rgba(251,146,60,0.25) !important; transform: translateY(-5px); border-color: rgba(251,146,60,0.4) !important; }
      `}</style>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '32px', textAlign: 'center' }}>
        ...
        <div className="card-hover card-total" style={{ background: 'rgba(56, 189, 248, 0.05)', border: '1px solid rgba(56, 189, 248, 0.2)', padding: '24px 48px', borderRadius: '24px', boxShadow: '0 8px 32px rgba(0,0,0,0.1)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '24px' }}>
        ...
        {/* Virements */}
        <div className="card-hover card-virement" style={{ background: 'rgba(56,189,248,0.05)', border: '1px solid rgba(56,189,248,0.15)', padding: '24px', borderRadius: '20px' }}>
        ...
        {/* Espèces */}
        <div className="card-hover card-espece" style={{ background: 'rgba(250,204,21,0.05)', border: '1px solid rgba(250,204,21,0.15)', padding: '24px', borderRadius: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        {/* Mobile Money */}
        <div className="card-hover card-mm" style={{ background: 'rgba(251,146,60,0.05)', border: '1px solid rgba(251,146,60,0.15)', padding: '24px', borderRadius: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
