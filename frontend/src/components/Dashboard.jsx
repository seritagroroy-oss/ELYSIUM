...
import CancelSortantModal from './modals/CancelSortantModal';
  ...
  return (
    <div onClick={() => { setContextMenu(null); setReposMenu(null); }}>
          ...
          onAction={async (code, ctx) => {
            ...
            if (code === 'T') {
              ...
              if (agent) {
                ...
                setEntrantDate(agent.hire_date || agent.entry_date || ctx.dateKey || formatDateKey(datesList[0]));
            ...
            } else if (code === 'DELETE_ENTRANT') {
              if (agent) {
                setCancelEntrantModalData(agent);
              }
            } else if (code === 'DELETE_SORTANT') {
              if (agent) {
                setCancelSortantModalData(agent);
              }
            } else {
      ...
      {siteData.length > 0 && showKPICards && activeSiteId !== 'site_administration' && (
        <DashboardKPI
          selectedKpiAgent={selectedKpiAgent}
          setSelectedKpiAgent={setSelectedKpiAgent}
          isScrolled={isScrolled}
          handleKpiMouseDown={handleKpiMouseDown}
          kpiPos={kpiPos}
          setKpiPos={setKpiPos}
          isDraggingKpi={isDraggingKpi}
      ...
      {cancelEntrantModalData && (
        <CancelEntrantModal
          ...
          loadSiteData={() => loadDashboardData(true)}
        />
      )}

      {cancelSortantModalData && (
        <CancelSortantModal
          cancelSortantModalData={cancelSortantModalData}
          setCancelSortantModalData={setCancelSortantModalData}
          period={period}
          setSiteData={setSiteData}
          loadSiteData={() => loadDashboardData(true)}
        />
      ...
      {deleteAgentConfirm && (
        <DeleteAgentModal
          agent={typeof deleteAgentConfirm === 'object' ? deleteAgentConfirm : { id: deleteAgentConfirm }}
