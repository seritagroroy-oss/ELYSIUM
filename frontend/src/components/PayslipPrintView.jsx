...
export default function PayslipPrintView({ onClose }) {
  ...
  // --- Mode Actuel / Archives (Bulletins) ---
  const [bulletinMode, setBulletinMode] = useState('actuel');
  const [bulletinCurrentPeriod, setBulletinCurrentPeriod] = useState('');
  const [bulletinIsCloture, setBulletinIsCloture] = useState(false);
  const [bulletinArchivedPeriods, setBulletinArchivedPeriods] = useState([]);
  const [bulletinSelectedArchive, setBulletinSelectedArchive] = useState('');
  const [bulletinArchiveData, setBulletinArchiveData] = useState([]);
  const [bulletinArchiveLoading, setBulletinArchiveLoading] = useState(false);
  ...
  // --- Fetch current period snapshot (Actuel) ---
  const loadData = async (targetPeriod) => {
    const p = targetPeriod || bulletinCurrentPeriod;
    if (!p) return;
    setLoading(true);
    try {
      const res = await apiCall('get_salaries', { period: p, scope: 'company' }, 'GET');
      if (Array.isArray(res)) {
        const list = res.map(ag => {
          ...
          const brut = Math.max(0, baseSalary - deductions + spGains + primeSite);
          const cnps = Math.round(brut * 0.063);
          const totalDeductions = deductions + (Number(ag.remboursement_pret) || 0);
          const netToPay = Math.max(0, brut - cnps - (Number(ag.remboursement_pret) || 0));

          return {
            ...ag,
            baseSalary,
            baseFull,
            absDays: ag.absences || 0,
            spCount: ag.sp_count || 0,
            deductions,
            spGains,
            cnps,
            brut,
            ...
            netToPay,
            primes: primeSite,
            acomptes: Number(ag.remboursement_pret) || 0,
            hire_date: ag.profile_data?.date_embauche || ag.profile_data?.hire_date || ''
          };
        });
  ...
  // --- Load published/archived periods info on mount ---
  const loadBulletinMeta = async () => {
    try {
      ...
      if (res?.success) {
        ...
        setBulletinCurrentPeriod(currentPeriod);
        setBulletinIsCloture(clotures.includes(currentPeriod));
        // Archives = mois passés archivés SAUF le mois en cours
        const pastArchs = allArchs.filter(p => p !== currentPeriod).sort().reverse();
        setBulletinArchivedPeriods(pastArchs);
        if (pastArchs.length > 0) setBulletinSelectedArchive(pastArchs[0]);
        // Charger les données du mois en cours
        if (currentPeriod) loadData(currentPeriod);
      }
    } catch (e) { console.error(e); }
  };

  useEffect(() => { loadBulletinMeta(); }, []);

  // Chargement du detail d'archive quand mode archives + periode choisie
  useEffect(() => {
    if (bulletinMode === 'archives' && bulletinSelectedArchive) {
      const fetchArchive = async () => {
        setBulletinArchiveLoading(true);
        try {
          const res = await apiCall(`get_payroll_archive_detail&period=${bulletinSelectedArchive}&scope=company`, {}, 'GET');
          if (res?.success && Array.isArray(res.archive?.salaries)) {
            const list = res.archive.salaries.map(ag => {
              const baseSalary = Number(ag.base) || 0;
              const deductions = Number(ag.deductions) || 0;
              const spGains = Number(ag.gains) || 0;
              const primeSite = Number(ag.prime_site) || 0;
              ...
              return {
                ...ag,
                baseSalary,
                baseFull: Number(ag.base_full) || baseSalary,
                absDays: ag.absences || 0,
                spCount: ag.sp_count || 0,
                deductions,
                spGains,
                cnps,
                brut,
                totalDeductions: deductions + (Number(ag.remboursement_pret) || 0),
                netToPay,
                primes: primeSite,
                acomptes: Number(ag.remboursement_pret) || 0,
                hire_date: ag.profile_data?.date_embauche || ag.profile_data?.hire_date || ''
            ...
            setBulletinArchiveData(list);
            setSelectedAgentIds(list.map(a => a.id));
          }
        } catch (e) { console.error(e); }
        finally { setBulletinArchiveLoading(false); }
      };
      fetchArchive();
    } else if (bulletinMode === 'actuel') {
      setSelectedAgentIds(agentsData.map(a => a.id));
    }
  }, [bulletinMode, bulletinSelectedArchive]);

  // --- Filtering (mode-aware) ---
  const activeData = bulletinMode === 'archives' ? bulletinArchiveData : agentsData;
  const filteredAgents = activeData.filter(ag => {
  ...
  const uniqueSites = ['ALL', ...new Set(activeData.map(a => a.site))];
        ...
        {toast && (
          <motion.div 
            ...
            style={{
              ...
              display: 'flex', flexDirection: 'column', gap: '8px', boxShadow: '0 10px 30px rgba(0,0,0,0.3)'
            }}
          >
            <span style={{ fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.5px', opacity: 0.9, fontWeight: 'bold' }}>JARVIS ASSISTANT</span>
      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '1.5rem', marginBottom: '2rem' }} className="no-print">
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          ...
          <h2 style={{ margin: 0, fontWeight: 700, fontSize: '1.6rem', color: '#f8fafc', textShadow: '0 0 10px rgba(52, 211, 153, 0.3)', display: 'flex', alignItems: 'center', gap: '12px' }}>
            <FileText style={{ color: '#34d399' }} size={28} /> Bulletins de Paie
          </h2>
          {/* Toggle Actuel / Archives */}
          <div style={{ display: 'flex', background: 'rgba(0,0,0,0.3)', padding: '4px', borderRadius: '12px', gap: '2px' }}>
            <button
              onClick={() => setBulletinMode('actuel')}
              style={{ padding: '6px 16px', borderRadius: '8px', border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: '0.85rem', transition: 'all 0.2s', background: bulletinMode === 'actuel' ? '#34d399' : 'transparent', color: bulletinMode === 'actuel' ? '#0f172a' : 'rgba(255,255,255,0.5)' }}
            >
              Actuel
            </button>
            <button
              onClick={() => setBulletinMode('archives')}
              style={{ padding: '6px 16px', borderRadius: '8px', border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: '0.85rem', transition: 'all 0.2s', background: bulletinMode === 'archives' ? '#34d399' : 'transparent', color: bulletinMode === 'archives' ? '#0f172a' : 'rgba(255,255,255,0.5)', display: 'flex', alignItems: 'center', gap: '6px' }}
            >
              🗄️ Archives
            </button>
          </div>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          ...
          <button className="btn btn-primary" onClick={executePrint} disabled={(bulletinMode === 'actuel' && !bulletinIsCloture) || selectedAgentIds.length === 0} style={{ background: '#34d399', color: '#0f172a', border: 'none', borderRadius: '12px', padding: '12px 24px', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '10px', boxShadow: '0 4px 20px rgba(52, 211, 153, 0.3)', opacity: ((bulletinMode === 'actuel' && !bulletinIsCloture) || selectedAgentIds.length === 0) ? 0.5 : 1, cursor: ((bulletinMode === 'actuel' && !bulletinIsCloture) || selectedAgentIds.length === 0) ? 'not-allowed' : 'pointer' }}>
            <Printer size={20} /> Imprimer les Bulletins ({(bulletinMode === 'actuel' && !bulletinIsCloture) ? 0 : selectedAgentIds.length})
          </button>
        </div>
      </header>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '30px', flex: 1, overflow: 'hidden' }} className="no-print">
        ...
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', padding: '1.75rem', borderRadius: '24px', display: 'flex', flexDirection: 'column', gap: '24px', overflowY: 'auto' }}>
          <div>
            ...
            {/* Period / Archive Selector */}
            <div style={{ marginBottom: '15px' }}>
              <label style={{ display: 'block', fontSize: '0.85rem', color: '#64748b', marginBottom: '6px', fontWeight: 600 }}>
                {bulletinMode === 'archives' ? 'Mois archivé' : 'Mois en cours'}
              </label>
              {bulletinMode === 'actuel' ? (
                <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', padding: '10px 12px', borderRadius: '10px' }}>
                  <div style={{ fontWeight: 700, color: bulletinIsCloture ? '#22c55e' : '#fbbf24', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span>{bulletinIsCloture ? '✅' : '🔄'}</span>
                    <span>{bulletinCurrentPeriod || 'Chargement...'}</span>
                  </div>
                  <div style={{ fontSize: '0.75rem', color: '#64748b', marginTop: '4px' }}>
                    {bulletinIsCloture ? 'Clôturé — données définitives' : 'En cours de traitement'}
                  </div>
                </div>
              ) : (
                <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', padding: '8px 12px', borderRadius: '10px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <select value={bulletinSelectedArchive} onChange={(e) => { setBulletinSelectedArchive(e.target.value); }} style={{ background: 'transparent', color: 'white', border: 'none', outline: 'none', cursor: 'pointer', fontWeight: 600, width: '100%' }}>
                    {bulletinArchivedPeriods.length === 0 && <option value="">Aucune archive</option>}
                    {bulletinArchivedPeriods.map(p => <option key={p} value={p} style={{ background: '#1e293b', color: 'white' }}>{p}</option>)}
                  </select>
                </div>
              )}
            </div>
        ...
        {/* Right pane: list of agents table OR Banner */}
        <div style={{ background: 'rgba(255,255,255,0.01)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '24px', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          {(bulletinMode === 'actuel' && !bulletinIsCloture) ? (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 40px', textAlign: 'center' }}>
              {!bulletinCurrentPeriod ? (
                <>
                  <div style={{ fontSize: '4rem', marginBottom: '16px', opacity: 0.3 }}>📋</div>
                  <p style={{ fontSize: '1.1rem', color: 'rgba(255,255,255,0.6)' }}>Aucune période publiée.<br />Publiez d'abord un pointage depuis le module Pointage.</p>
                </>
              ) : (
                <>
                  <div style={{ fontSize: '5rem', marginBottom: '20px' }}>⏳</div>
                  <h3 style={{ fontWeight: 800, fontSize: '1.5rem', margin: '0 0 10px 0', color: '#fbbf24' }}>
                    {bulletinCurrentPeriod} — En cours de traitement
                  </h3>
                  <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '1rem', maxWidth: '480px', margin: '0 auto 24px auto', lineHeight: '1.6' }}>
                    Les bulletins sont en cours de traitement par le comptable. Ils seront disponibles pour impression ici une fois le mois clôturé.
                  </p>
