...
export default function ArchivesPointage({ onSwitchToCurrent, setView }) {
  ...
  const [checkingNavState, setCheckingNavState] = useState(true);
  // ✅ Signal atomique : ne passe à true qu'une fois que setArchives() ET setLoading(false)
  // ont tous les deux été appliqués dans le même cycle React — évite la race condition.
  ...
  useEffect(() => {
    ...
    window.addEventListener('closeArchiveMode', handleCloseArchive);
    ...
    return () => {
      ...
      window.removeEventListener('closeArchiveMode', handleCloseArchive);
      if (fallbackTimer) clearTimeout(fallbackTimer);
    };
  }, [isAutoSearching]);

  // NOUVEAU — Charger nav_state depuis la session PHP au montage
  useEffect(() => {
    const fetchNavState = async () => {
      try {
        const res = await apiCall('get_nav_state', {}, 'GET');
        if (res.success && res.nav_state && res.nav_state.period) {
          setNavState(res.nav_state);
          setIsAutoSearching(true); // afficher l'overlay
        }
      } catch (e) {
        console.error('get_nav_state error:', e);
      } finally {
        setCheckingNavState(false);
      }
    };
    fetchNavState();
  }, []);
  ...
  const loadArchives = async () => {
    setLoading(true);
    setIsLoadingComplete(false); // reset du signal avant chaque chargement
    try {
      ...
      if (res.success && Array.isArray(res.archives)) {
        ...
        // ✅ Grouper setArchives + setLoading(false) + signal de complétion dans le même
        // appel synchrone pour éviter que le useEffect se déclenche avec une liste partielle.
        setArchives(archivesList);
        setLoading(false);
        setIsLoadingComplete(true);
        return; // court-circuit le finally pour éviter double setLoading(false)
      }
    } catch (e) {
      console.error(e);
    }
    // Fallback : erreur ou res.success === false
    setLoading(false);
    setIsLoadingComplete(true);
  };
  ...
  useEffect(() => {
    // ✅ N'agir QUE quand isLoadingComplete est true : garantit que la liste des archives
    // est DÉFINITIVE (après syncOldArchives + resyncEmptyArchives + dernier GET).
    if (!isLoadingComplete) return;
    if (!navState || !navState.period) return;

    try {
      const targetArchive = archives.find(a => a.period === navState.period);
      if (targetArchive) {
        // Archive trouvée → naviguer directement vers le pointage de l'agent
        handleViewArchive(targetArchive.id);
      } else {
        // Archive absente de la liste FINALE → période non encore publiée
        console.warn(`[ArchivesPointage] Archive ${navState.period} introuvable dans la liste finale des archives.`);
        setIsAutoSearching(false);
        setNavState(null);
      }
    } catch (e) {
      console.error('Error handling navState search:', e);
      setIsAutoSearching(false);
      setNavState(null);
    }
  }, [isLoadingComplete, archives, navState]);
  ...
  const renderOverlay = () => {
    if (!isAutoSearching && !checkingNavState) return null;
    return createPortal(
  ...
  if (selectedArchiveId && archiveDetail) {
    return (
      <div style={{ position: 'relative', width: '100%', height: '100%' }}>
        ...
        <Dashboard 
          isArchiveMode={true} 
          archiveData={archiveDetail} 
          setView={setView} 
          navState={navState} 
          onBack={() => { setSelectedArchiveId(null); setArchiveDetail(null); }}
