...
$stmt = $sqlite->query("SELECT service_id, data_value FROM service_data WHERE data_key = 'published_periods'");
$rows = $stmt; // ElysiumPdoDb->query returns array
...
// 2. Force delete snapshot for ALL companies
$stmt = $sqlite->prepare("DELETE FROM payroll_snapshots WHERE period = ?");
$stmt->execute([$period]);
echo "Deleted from payroll_snapshots. Rows: " . $stmt->rowCount() . "<br/>\n";

// 3. Force delete archives for ALL companies
$stmt = $sqlite->prepare("DELETE FROM archives WHERE period = ? OR id = ?");
