...
console.log("Applying transfer logic safely...");
...
// Remove old TransferModal render block accurately!
const startStr = "{transferModal && (";
const endStr = "/>\n      )}"; // The exact end of the block
const startIndex = dCode.indexOf(startStr);
if (startIndex !== -1) {
    const endIndex = dCode.indexOf(endStr, startIndex) + endStr.length;
    dCode = dCode.substring(0, startIndex) + dCode.substring(endIndex);
}
